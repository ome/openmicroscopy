/*
 * org.openmicroscopy.shoola.agents.viewer.controls.TNavigatorManager
 *
 *------------------------------------------------------------------------------
 *
	...
	implements ActionListener, FocusListener, ChangeListener
{
	...
	*/
	 ...
	 * enters some text. If the entered text can be  converted to a valid 
	 * timepoint, the {@link #synch(int) synch} method is invoked in order to 
	 * set all elements to the new timepoint value. 
	...
	/** Handle events fired by slider and spinner. */
	public void stateChanged(ChangeEvent e)
	{
		Object src = e.getSource();
		if (src == view.getTSlider()) {
			int val = view.getTSlider().getValue();
			//user's moving slider knob
			//if (view.getTSlider().getValueIsAdjusting())  
				//synch(val, false);
			//else  // user finished moving slider knob
				synch(val);  
		} else {  // spinner
	...
	public void focusLost(FocusEvent e)
	{
		...
		if (val == null || !val.equals(t)) view.getTField().setText(t);
	}
