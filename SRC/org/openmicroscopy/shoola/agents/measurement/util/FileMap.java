...
{	
	...
	{
		ArrayList<IXMLElement> servers = parent.getChildrenNamed(IOConstants.SERVER_TAG);
		for(IXMLElement node : servers)
		{
			if(node.getAttribute(IOConstants.SERVER_NAME_ATTRIBUTE, "").equals(server))
				return node;
		}
		return null;
	}
	...
	{
		ArrayList<IXMLElement> users = parent.getChildrenNamed(IOConstants.USER_TAG);
		for(IXMLElement node : users)
		{
			if(node.getAttribute(IOConstants.USER_NAME_ATTRIBUTE, "").equals(user))
				return node;
		}
		return null;
	}
