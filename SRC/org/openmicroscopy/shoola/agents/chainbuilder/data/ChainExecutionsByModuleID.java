/*
 * org.openmicroscopy.shoola.agents.chainbuilder.data.ChainExecutionsByModuleId
 *
 *------------------------------------------------------------------------------
 *
...
* @since OME2.2
*/
public class ChainExecutionsByModuleID {
	
	private HashMap map = null;
	
	public ChainExecutionsByModuleID(Collection execs) {
		if (execs == null) 
			return;
	...
	private void addNodeExecution(NodeExecutionData exec) {
		ModuleExecutionData modExec = exec.getModuleExecution();
		ModuleData module = (ModuleData) modExec.getModule();
		Integer id = new Integer(module.getID());
		
		// get the hash contents for this module
		Object obj = map.get(id);
		Vector mexes; // the vector that will contain the mexes for this 
					// module
		if (obj == null) 
			mexes = new Vector();
		else
	...
	/** 
	 * Get the list of mexes for a module with a given id
	 * @param id
	 * @return
	 */
	public Vector getMexes(int id) {
		if (!hasExecs())
			return null;
	 ...
	 * @return
	 */
	public Vector getMexes(ModuleData module) {
		return getMexes(module.getID());
	}
