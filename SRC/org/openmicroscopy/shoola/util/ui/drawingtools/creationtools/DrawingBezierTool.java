		...
		implements DrawingCreationTool
{
	 ...
	 */
	private BezierAnnotationFigure		prototype;
	
	/**
	 * The created figure.
	 */
	protected BezierAnnotationFigure		createdFigure;
	...
	public void mousePressed(MouseEvent evt)
	{
		...
		if (createdFigure==null)
		{
			...
			createdFigure.addNode(new BezierPath.Node(getView()
				.getConstrainer().constrainPoint(
	...
	protected BezierAnnotationFigure createFigure()
	{
		BezierAnnotationFigure f=(BezierAnnotationFigure) prototype.clone();
	...
	protected BezierAnnotationFigure getCreatedFigure()
	{
		return createdFigure;
	}
	
	protected BezierAnnotationFigure getAddedFigure()
	{
		return createdFigure;
	}
	...
	protected void addPointToFigure(Point2D.Double newPoint)
	{
		...
		else
		{
			...
			Point2D.Double secondLastPoint=
					(pointCount<=1) ? endPoint : createdFigure.getPoint(
						pointCount-2, 0);
			if (newPoint.equals(endPoint))
			{
				// nothing to do
					...
					&&Geom.lineContainsPoint(newPoint.x, newPoint.y,
						secondLastPoint.x, secondLastPoint.y, endPoint.x,
						endPoint.y, 0.9f/getView().getScaleFactor()))
			{
		...
		if (createdFigure!=null)
		{
			switch (evt.getClickCount())
			{
				case 1:
					...
					{
						Rectangle r=
								new Rectangle(getView().drawingToView(
	...
	public void mouseReleased(MouseEvent evt)
	{
		if (finishWhenMouseReleased==Boolean.TRUE)
		{
			if (createdFigure.getNodeCount()>2)
			{
				BezierPath fittedPath=
						Bezier.fitBezierCurve(createdFigure.getBezierPath(), 1);
				createdFigure.willChange();
	...
	protected void finishCreation(BezierAnnotationFigure createdFigure)
	{
		getView().addToSelection(createdFigure);
	}
