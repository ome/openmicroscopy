 /*
 * org.openmicroscopy.shoola.agents.editor.browser.actions.UndoAction 
 *
 *------------------------------------------------------------------------------
...
/** 
 * This Action performs an Undo on the {@link #undoManager} specified in the 
 * constructor. 
 * It listens both to the posting of {@link UndoableEdit}s to the undoSupport
 * and to the undo and redo actions of the {@link #undoManager}.
	...
	UndoRedoListener 
{
			...
			UndoableEditSupport undoSupport, Browser model) 
	{
		...
		putValue(Action.SMALL_ICON, IconManager.getInstance().getIcon(
				IconManager.UNDO_ICON)); 
	...
	private void refreshStatus() {
		...
		setEnabled(enabled);
		setDescription(
				enabled ? undoManager.getUndoPresentationName() : "Can't Undo");
	}
