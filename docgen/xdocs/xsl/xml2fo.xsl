		...
		<fo:root>
			...
			<fo:layout-master-set>
				<fo:simple-page-master master-name="cover">
					<!-- left margin -->
					<xsl:attribute name="margin-left"><xsl:choose><xsl:when test="$dfi/cover-left-margin"><xsl:value-of select="$dfi/cover-left-margin"/></xsl:when><xsl:otherwise>40pt</xsl:otherwise></xsl:choose></xsl:attribute>
					<!-- right margin -->
					<xsl:attribute name="margin-right"><xsl:choose><xsl:when test="$dfi/cover-right-margin"><xsl:value-of select="$dfi/cover-right-margin"/></xsl:when><xsl:otherwise>40pt</xsl:otherwise></xsl:choose></xsl:attribute>
					<!-- top margin -->
					<xsl:attribute name="margin-top"><xsl:choose><xsl:when test="$dfi/cover-top-margin"><xsl:value-of select="$dfi/cover-top-margin"/></xsl:when><xsl:otherwise>50pt</xsl:otherwise></xsl:choose></xsl:attribute>
					<!-- bottom margin -->
					<xsl:attribute name="margin-bottom"><xsl:choose><xsl:when test="$dfi/cover-bottom-margin"><xsl:value-of select="$dfi/cover-bottom-margin"/></xsl:when><xsl:otherwise>25pt</xsl:otherwise></xsl:choose></xsl:attribute>
				...
				<xsl:if test="$generate-toc-page = 'yes'">
					<fo:simple-page-master master-name="toc">
						<!-- left margin -->
						<xsl:attribute name="margin-left"><xsl:choose><xsl:when test="$dfi/toc-left-margin"><xsl:value-of select="$dfi/toc-left-margin"/></xsl:when><xsl:otherwise>40pt</xsl:otherwise></xsl:choose></xsl:attribute>
						<!-- right margin -->
						<xsl:attribute name="margin-right"><xsl:choose><xsl:when test="$dfi/toc-right-margin"><xsl:value-of select="$dfi/toc-right-margin"/></xsl:when><xsl:otherwise>40pt</xsl:otherwise></xsl:choose></xsl:attribute>
						<!-- top margin -->
						<xsl:attribute name="margin-top"><xsl:choose><xsl:when test="$dfi/toc-top-margin"><xsl:value-of select="$dfi/toc-top-margin"/></xsl:when><xsl:otherwise>50pt</xsl:otherwise></xsl:choose></xsl:attribute>
						<!-- bottom margin -->
						<xsl:attribute name="margin-bottom"><xsl:choose><xsl:when test="$dfi/toc-bottom-margin"><xsl:value-of select="$dfi/toc-bottom-margin"/></xsl:when><xsl:otherwise>25pt</xsl:otherwise></xsl:choose></xsl:attribute>
						<!-- layout for the body part -->
						<fo:region-body margin-top="{substring-before($header-height, 'pt')+1}pt" margin-bottom="50pt"/>
				...
				<fo:simple-page-master master-name="body">
					<!-- left margin -->
					<xsl:attribute name="margin-left"><xsl:choose><xsl:when test="$dfi/left-margin"><xsl:value-of select="$dfi/left-margin"/></xsl:when><xsl:otherwise>40pt</xsl:otherwise></xsl:choose></xsl:attribute>
					<!-- right margin -->
					<xsl:attribute name="margin-right"><xsl:choose><xsl:when test="$dfi/right-margin"><xsl:value-of select="$dfi/right-margin"/></xsl:when><xsl:otherwise>40pt</xsl:otherwise></xsl:choose></xsl:attribute>
					<!-- top margin -->
					<xsl:attribute name="margin-top"><xsl:choose><xsl:when test="$dfi/top-margin"><xsl:value-of select="$dfi/top-margin"/></xsl:when><xsl:otherwise>50pt</xsl:otherwise></xsl:choose></xsl:attribute>
					<!-- bottom margin -->
					<xsl:attribute name="margin-bottom"><xsl:choose><xsl:when test="$dfi/bottom-margin"><xsl:value-of select="$dfi/bottom-margin"/></xsl:when><xsl:otherwise>25pt</xsl:otherwise></xsl:choose></xsl:attribute>
					<!-- layout for the body part -->
					<fo:region-body margin-top="{substring-before($header-height, 'pt')+1}pt" margin-bottom="50pt">
	...
	<xsl:template match="hr">
		<fo:block>
			<fo:leader leader-pattern="rule" leader-length="100%" space-before.optimum="2pt" space-after.optimum="2pt">
				...
				<xsl:if test="@size">
					<xsl:attribute name="rule-thickness"><xsl:value-of select="@size"/>pt
</xsl:attribute>
				</xsl:if>
...
-->
	<xsl:template match="document-header">
		<fo:static-content flow-name="xsl-region-before">
			...
			<!-- Render hr after the header. TODO: factor out in template. -->
			<fo:block>
				<fo:leader leader-pattern="rule" leader-length="100%" space-before.optimum="2pt" space-after.optimum="2pt">
...
-->
	<xsl:template match="document-footer">
		<fo:static-content flow-name="xsl-region-after">
		    <!-- Render hr before the header. TODO: factor out in template. -->
			<fo:block>
				<fo:leader leader-pattern="rule" leader-length="100%" space-before.optimum="2pt" space-after.optimum="2pt">
					<xsl:attribute name="color">steelblue</xsl:attribute>	
					<xsl:attribute name="rule-thickness">1pt</xsl:attribute>
...
-->
	<xsl:template name="toc-link">
		...
		<fo:block space-before.optimum="5pt" text-align-last="justify" last-line-end-indent="-24pt">
			<xsl:attribute name="margin-left"><xsl:value-of select="$section-level - 1"/>em
</xsl:attribute>
			<fo:basic-link>
				<xsl:call-template name="get-css-for-element">
...
-->
	<xsl:template match="td">
		<fo:table-cell>
			...
			<xsl:if test="@valign">
				<xsl:attribute name="display-align"><xsl:choose><xsl:when test="@valign='bottom'">bottom</xsl:when><xsl:when test="@valign='middle'">center</xsl:when><xsl:otherwise>center</xsl:otherwise></xsl:choose></xsl:attribute>
			</xsl:if>
			<fo:block>
				<xsl:attribute name="text-align"><xsl:choose><xsl:when test="@align='right'">end</xsl:when><xsl:when test="@align='center'">center</xsl:when><xsl:when test="@align='justify'">justify</xsl:when><xsl:otherwise>start</xsl:otherwise></xsl:choose></xsl:attribute>
				<xsl:apply-templates/>
			</fo:block>
		</fo:table-cell>
	</xsl:template>
...
-->
	<xsl:template name="get-alignment">
		...
		<xsl:if test="$align">
			...
			<xsl:attribute name="text-align"><xsl:choose><xsl:when test="$alignment = 'right'">end</xsl:when><xsl:when test="$alignment = 'center'">center</xsl:when><xsl:otherwise>start</xsl:otherwise></xsl:choose></xsl:attribute>
		</xsl:if>
	</xsl:template>
	<!-- B: SECTION FUNCTIONS -->
	<!--
