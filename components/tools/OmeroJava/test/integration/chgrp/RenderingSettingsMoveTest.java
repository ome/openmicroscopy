...
public class RenderingSettingsMoveTest extends AbstractServerTest {
     ...
     */
    @Test
    public void testMoveImageWithRenderingSettings() throws Exception {
        String perms = "rw----";
    ...
    /**
     * Test to move an image with rendering settings from 2 users. Context: - 2
     * users view the image. - Owner move the image to a RW---- group. - The
     * settings of the second user should not be moved to the destination group.
     *
     * @throws Exception
     *             Thrown if an error occurred.
     */
    @Test
    public void testMoveImageWithRenderingSettingsRW() throws Exception {
        ...
        ParametersI param = new ParametersI();
        param.addLong("pid", pixels.getId().getValue());
        List<IObject> settingsUser1 = factory.getPixelsService()
                .retrieveAllRndSettings(pixels.getId().getValue(), ctx.userId);
        ...
        prx.setOriginalSettingsInSet(Pixels.class.getName(),
                Arrays.asList(pixels.getId().getValue()));
        List<IObject> settingsUser2 = factory.getPixelsService()
                .retrieveAllRndSettings(pixels.getId().getValue(), ctx2.userId);
    ...
    /**
     * Test to move an image with rendering settings from 2 users. Context: - 2
     * users view the image. - Owner move the image to a RWR--- group. - The
     * settings of the second user should be moved to the destination group.
     *
     * @throws Exception
     *             Thrown if an error occurred.
     */
    @Test
    public void testMoveImageWithRenderingSettingsRWR() throws Exception {
