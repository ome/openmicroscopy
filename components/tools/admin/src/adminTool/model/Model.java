...
public class Model 
{
	...
	public Model(OMEROMetadataStore store, String user)
	...
	public boolean isDefaultGroup(long groupID)
		throws IAdminException, UnknownException, PermissionsException
	{
		try 
		{
	...
	public Experimenter getUser(long i)
		throws IAdminException, UnknownException, PermissionsException
	{
		try
		{
	...
	public void update(Experimenter user, String name) throws 
				IAdminException, UnknownException, PermissionsException
	{
		try
		{
	...
	public void update(ExperimenterGroup group, String name) throws 
				IAdminException, UnknownException, PermissionsException
	{
		try
		{
			...
			throws IAdminException, UnknownException, PermissionsException
	{
		for(int i = 0 ; i < userList.size(); i++)
		{
			try
			{
				Experimenter user = iAdmin.lookupExperimenter((String)userList.get(i));
	...
	public void addGroupToUser(long group)
		throws IAdminException, UnknownException, PermissionsException
	{
		try
		{
	...
	public void removeGroupFromUser(long group)
		throws IAdminException, UnknownException, PermissionsException
	{
		try
		{
			if(userBelongsToGroup(currentUser, group))
				iAdmin.removeGroups(iAdmin.getExperimenter(currentUser), 
					iAdmin.getGroup(group));
		}
		catch(Exception e)
		{
	...
	public List<String> getGroupsUserNotMemberOf(String userName)
	{
		...
		List<String> userGroupList =
			getUserGroupMembership(iAdmin.lookupExperimenter(userName).getId());
