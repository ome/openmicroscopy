...
import adminTool.ui.PasswordDialog;
	...
	implements UserListController
{
	...
	void setSystemUser()
	{
		...
		try
		{
			...
			{
				throw new IAdminException(new Exception("Cannot find user in " +
	...
	void setDefaultGroup()
	{
		...
		try
		{
			...
			{
				throw new IAdminException(new Exception("Cannot find group in " +
						"Database. Admintool may be out of sync with Database."));
			}
		}
		catch(Exception e)
	...
	void addNewUser()
	{
		...
		{
			view.showErrorMessage("You have already created a user called \'NewUser\'." +
	...
	/**
	 * Remove the group selected in the {@see UsersTab#groupList} from 
	 * the users list of groups. 
	 * Once complete, redisplay the users new settings. 
	 */
	public void removeGroupFromUser()
	{
		...
		try
		{
			...
			{
				view.showErrorMessage("Cannot remove default group from user." +
	...
	public void addUser()
	{
		...
		try
		{
			if(model.findUserByName("NewUser"))
			{
				view.showErrorMessage("A user called NewUser already exists. " +
	...
	void handleException(Exception e, String operation)
	{
		if( e instanceof IAdminException || e instanceof PermissionsException )
		{
			...
			if( e instanceof PermissionsException )
			{
				...
				String reason = "You do not have sufficient privileges to " + operation;
				Point location = view.getLocationOnScreen();
				location.x += view.getWidth()/2-200;
				location.y += view.getHeight()/2-150;
				dlg = new ErrorDialog(location, "Permissions Failure", e, reason);
			}
			else
			{
				...
				location.x += view.getWidth()/2-200;
				location.y += view.getHeight()/2-150;
				dlg = new ErrorDialog(location, "IAdmin Could not complete task", e, reason);			
			}
		}
		else
		{
			if( e instanceof UnknownException)
			{
				...
				DebugMessenger debug = new DebugMessenger(null,"An Unexpected " +
						"Error has Occurred", true, error.getException());
			}
			else
			{
				DebugMessenger debug = new DebugMessenger(null,"An Unexpected " +
