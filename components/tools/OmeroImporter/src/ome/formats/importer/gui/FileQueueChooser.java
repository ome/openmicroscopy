...
public class FileQueueChooser extends JFileChooser implements ActionListener
{
	...
	FileQueueChooser(ImportConfig config, OMEROWrapper scanReader) {
		...
		FileFilter[] originalFF = customFileFilters(scanReader.getImageReader());

		FileFilter combo = makeComboFilter(sortFilters(originalFF));
		if (combo != null) this.addChoosableFileFilter(combo);

		//loci.formats.gui.GUITools.buildFileFilters(scanReader.getImageReader());
		//FileFilter combo = null;
		for (int i = 0; i < originalFF.length; i++)
			this.addChoosableFileFilter(originalFF[i]);

		this.addChoosableFileFilter(new DashFileFilter());
		this.addChoosableFileFilter(new R3DNewFileFilter());
		this.addChoosableFileFilter(new R3DOldFileFilter());
		this.addChoosableFileFilter(new D3DNewFileFilter());
		this.addChoosableFileFilter(new D3DOldFileFilter());
		this.addChoosableFileFilter(new D3DNPrjFileFilter());
	...
	public static FileFilter[] customFileFilters(IFormatHandler handler) {
		...
		{
			ImageReader iReader = (ImageReader) handler;
			IFormatReader[] readers = iReader.getReaders();
			Vector filterList = new Vector();
			for (int i=0; i<readers.length; i++)
			{
				filterList.add(new ImageReaderFileFilter(readers[i]));
			}
	 ...
	 */
	@SuppressWarnings("unchecked")
	static class ImageReaderFileFilter extends javax.swing.filechooser.FileFilter
	implements Comparable
	{
		...
		public ImageReaderFileFilter(IFormatReader reader) {
			this.reader = reader;
			String[] exts = reader.getSuffixes();
			desc = reader.getFormat();
		...
		public boolean accept(File f) {
			...
			return reader.isThisType(f.getPath(), true);
		}
