  ...
  implements FocusListener, ItemListener, MouseListener
{
  ...
  public int promptId() {
    ...
    else if (isOME()) return promptIdOME();
    else return promptIdOMERO(); // isOMERO
  }
...
// begin OMERO additions 
  ...
  public int promptIdOMERO() 
  {
    if (id == null) {
        String server = Prefs.get("downloader.server", "");
        String user = Prefs.get("downloader.user", "");
        String port = Prefs.get("downloader.port", "4063");
        String pass = null;
        GenericDialog gd = new GenericDialog("OMERO Login");

        IJ.showStatus("Connect to OMERO");
        ...
        IJ.showStatus("Establishing OMERO connection...");
        ServiceFactoryPrx theServiceFactory = 
        	createConnectionAndServiceFactory(
        			server,
        			portInt,
        			user,
        			pass);
        IJ.showStatus("Locating Image in OMERO server...");
        Project theProject = promptForProject(theServiceFactory);
        Dataset theDataset = promptForDataset(theServiceFactory, theProject);
        Image theImage = promptForImage(theServiceFactory, theDataset);
        String theSessionId;
        try {
        	theSessionId = theServiceFactory.getAdminService().getEventContext().sessionUuid;
		} catch (ServerError e) {
			theSessionId = "";
		}
        if (theImage == null)
        {
            id = server + "?port=" + port + "&username=" + user + "&session=" + theSessionId;        	
        } else {
            id = server + "?port=" + port + "&username=" + user + "&session=" + theSessionId + "&id=" + theImage.getId().getValue();        	
        }
    }
    
    idType = "OMERO address";
    return STATUS_OK;
  }

private ServiceFactoryPrx createConnectionAndServiceFactory(
		  ...
		  String thePassword)
  {
	  ...
	  try
	  {
		  ...
		  client theClient = new client(theServerName, thePort);
	  ...
	  {
		  System.out.println("Exception in createConnectionAndServiceFactory");
		  e.printStackTrace();
	  }
	  return null;
  }

  private Project promptForProject(ServiceFactoryPrx theServiceFactory)
  {
	  Project theSelectedProject = null;
	  try
	  {
	      ...
	      GenericDialog gd = new GenericDialog("Select Project");

	      String[] theProjectNames = new String[theProjectList.size()];
	      
	      HashMap<String, Project> theProjectHash = new HashMap<String, Project>();
		  int theIndex = 0;
		  ...
		  for  (Project aProject : theProjectList)
		  {
			  ...
			  if (aProject.getName() != null)
			  {
				  ...
				  theDisplayName = aProject.getName().getValue() + " : " + aProject.getId().getValue();
			  } else {
				  theDisplayName = "No Name" + " : " + aProject.getId().getValue();
			  }
			  theProjectNames[theIndex++] = theDisplayName;
			  theProjectHash.put(theDisplayName, aProject);
		  }
		  
	      gd.addChoice("Project: ", theProjectNames, theProjectNames[0]);
	      ...
	      theSelectedProject = theProjectHash.get(gd.getNextChoice());
	  ...
	  if (theSelectedProject != null && theSelectedProject.getName() != null)
	  {
		  System.out.println("Selected Project:" + theSelectedProject.getName().getValue());
	  }
	  return theSelectedProject;
  }

  private Dataset promptForDataset(ServiceFactoryPrx theServiceFactory, Project theProject)
  {
  	  Dataset theSelectedDataset = null;
  	  try
  	  {
  		  System.out.println("Getting IContainer Proxy");
  		  IContainerPrx theContainer = 
  			  theServiceFactory.getContainerService();
  		  List<Dataset> theDatasetList = getDatasets(theContainer, theProject);

  	      GenericDialog gd = new GenericDialog("Select Dataset");

  	      String[] theDatasetNames = new String[theDatasetList.size()];
  	      
  	      HashMap<String, Dataset> theDatasetHash = new HashMap<String, Dataset>();
  		  int theIndex = 0;
  		  String theDisplayName;
  		  for  (Dataset aDataset : theDatasetList)
  		  {
  			  System.out.println("Dataset Info:");
  			  if (aDataset.getName() != null)
  			  {
  				  System.out.println(aDataset.getName().getValue());
  				  theDisplayName = aDataset.getName().getValue() + " : " + aDataset.getId().getValue();
  			  } else {
  				  theDisplayName = "No Name" + " : " + aDataset.getId().getValue();
  			  }
  			  theDatasetNames[theIndex++] = theDisplayName;
  			  theDatasetHash.put(theDisplayName, aDataset);
  		  }
  		  
  	      gd.addChoice("Dataset: ", theDatasetNames, theDatasetNames[0]);
  	      ...
  	      theSelectedDataset = theDatasetHash.get(gd.getNextChoice());
  	  ...
  	  if (theSelectedDataset != null && theSelectedDataset.getName() != null)
  	  {
  		  System.out.println("Selected Dataset:" + theSelectedDataset.getName().getValue());
  	  }
  	  return theSelectedDataset;
  }

  private Image promptForImage(ServiceFactoryPrx theServiceFactory, Dataset theDataset)
  {
  	  Image theSelectedImage = null;
  	  try
  	  {
  		  System.out.println("Getting IContainer Proxy");
  		  IContainerPrx theContainer = 
  			  theServiceFactory.getContainerService();
  		  List<Image> theImageList = getImages(theContainer, theDataset);

  		  if (theImageList.size() == 0)
  		  {
  			IJ.error("No Images were found in this dataset.");
  			return null;
  		  }
  		  
  	      GenericDialog gd = new GenericDialog("Select Image");

  	      String[] theImageNames = new String[theImageList.size()];
  	      
  	      HashMap<String, Image> theImageHash = new HashMap<String, Image>();
  		  int theIndex = 0;
  		  String theDisplayName;
  		  for  (Image aImage : theImageList)
  		  {
  			  System.out.println("Image Info:");
  			  if (aImage.getName() != null)
  			  {
  				  System.out.println(aImage.getName().getValue());
  				  theDisplayName = aImage.getName().getValue() + " : " + aImage.getId().getValue();
  			  } else {
  				  theDisplayName = "No Name" + " : " + aImage.getId().getValue();
  			  }
  			  theImageNames[theIndex++] = theDisplayName;
  			  theImageHash.put(theDisplayName, aImage);
  		  }
  		  
  	      gd.addChoice("Image: ", theImageNames, theImageNames[0]);
  	      ...
  	      theSelectedImage = theImageHash.get(gd.getNextChoice());
  	  ...
  	  if (theSelectedImage != null && theSelectedImage.getName() != null)
  	  {
  		  System.out.println("Selected Image:" + theSelectedImage.getName().getValue());
  	  }
  	  return theSelectedImage;
  }  
  ...
  public List<Dataset> getDatasets(IContainerPrx iContainer, Project p)
  {
  	try
  	{
  ...
  public List<Image> getImages(IContainerPrx iContainer, Dataset d)
  {
	  try
	  {
		  List<Long> ids = new ArrayList<Long>(1);
		  ...
		  if (objects.size() > 0)
		  {
			  Dataset dataset = (Dataset) objects.get(0);
			  return dataset.linkedImageList();
		  }
		  return null;
	  }
	  catch (ServerError e)
...
/* add call to closeOmeroClient
  
  private void closeOmeroClient(client theClient)
  {
	  if (theClient!= null)
	  {
