...
$.fn.sdialog = function(cfg) {
  return this.each(function(){
    ...
    this.placeAround = function (elm) {
      ...
      epos.left -= (size.w/2) - ($(elm).width() / 2);
      if (epos.left + size.w > wsize.w) {
        epos.left = wsize.w - size.w - 10;
      }
      self.css({top: epos.top - size.h, left: epos.left});
      this.recalcSize();
    }
