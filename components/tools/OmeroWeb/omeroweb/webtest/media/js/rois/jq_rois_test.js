$(document).ready(function() {
    ...
    var plot_rois = function() {
        ...
        $canvas.empty();
        var settings = {color: '#ff0000', stroke: 2}
            ...
            for (var s=0; s<shapes.length; s++) {
                ...
                if ((shape['theT'] == theT) && (shape['theZ'] == theZ)) {
                    if (shape['type'] == 'Ellipse') {
                        $canvas.drawEllipse(shape['cx'], shape['cy'], shape['rx'], shape['ry'], settings);
                    }
                    else if (shape['type'] == 'Rectangle') {
                        $canvas.drawRect(shape['x'], shape['y'], shape['width'], shape['height'], settings);
                    }
                    else if (shape['type'] == 'Point') {
                        $canvas.drawEllipse( shape['cx'], shape['cy'], 2, 2, settings);
                    }
                }
            }
        }
