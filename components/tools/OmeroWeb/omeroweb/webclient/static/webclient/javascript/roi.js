...
$.imgAreaSelect.init = function (img, options) {
    ...
    function update(resetKeyPress)
    {
        ...
        // draw ROIs
        if (options.selectionShape=='elipse' || options.selectionShape=='rectangle' || options.selectionShape=='line') {
            $('canvas').remove();
            w = M.max(selection.width - options.borderWidth * 2, 0);
            h = M.max(selection.height - options.borderWidth * 2, 0);
            if (options.selectionShape=='elipse') {
                $area.drawEllipse(0, 0, w, h);
            } else if(options.selectionShape=='rectangle') {
                $area.drawRect(0, 0, w, h);
            } else if(options.selectionShape=='line') {
                if (x1>x2) {
                    if (y1>y2) {
                        $area.drawLine(0, 0, w, h);
                    } else {
                        $area.drawLine(w, 0, 0, h);
                    }
                } else {
                    if (y1>y2) {
                        $area.drawLine(w, 0, 0, h);
