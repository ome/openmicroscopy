...
class BaseContainer(BaseController):
    ...
    def canUseOthersAnns(self):
        ...
        """
        gid = self.conn.SOPTS.getOmeroGroup()
        if gid is None:
            return False
        try:
        ...
        for k in oids:
            if len(oids[k]) > 0:
                for ob in oids[k]:
                    ...
                    new_links.append(l_ann)
        if len(new_links) > 0 :
            new_links = self.conn.getUpdateService().saveAndReturnArray(new_links, self.conn.SOPTS)
    ...
    def createAnnotationsLinks(self, atype, tids, oids, well_index=0):
        ...
        try:
            ...
            saved_links = self.conn.getUpdateService().saveAndReturnArray(new_links, self.conn.SOPTS)
        except omero.ValidationException, x:
            for l in new_links:
                try:
                    saved_links.append(self.conn.getUpdateService().saveAndReturnObject(l, self.conn.SOPTS))
                except:
                    failed+=1
