...
# helper method
# TODO Surely this should throw an exception id there is an error other than
# the parameter being missing?
...
def load_template(request, menu, conn=None, url=None, **kwargs):
    ...
    # validate experimenter is in the active groups
...
def chgrp(request, conn=None, **kwargs):
    ...
    # Get the target container within the target group
    ...
    # Chgrp objects of each type in turn
    ...
    for dtype in dtypes:
        ...
        if oids is not None:
            ...
            if len(fsIds) > 0:
                ...
                # else:
                #     update_list.update(
                #         get_fileset_images_and_containers(
                #             conn,
                #             [long(x) for x in fsIds])
                #         )
            ...
            print('chgrp: ', group_id, dtype, obj_ids)
