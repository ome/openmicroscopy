...
class OmeroWebGateway (omero.gateway.BlitzGateway):
    ...
    def isAnythingCreated(self):
        ...
        if len(q.findAllByQuery(sql, p, self.SOPTS)) > 0:
            return False
        return True
    ...
    def findWellInPlate (self, plate_name, row, column):
        ...
        well = q.findByQuery(sql, p, self.SOPTS)
        if well is None:
            return None
        else:
    ...
    def listTags(self, eid=None):
        ...
        q = self.getQueryService()
        for ann in q.findAllByQuery(sql, params, self.SOPTS):
    ...
    def countOrphans (self, obj_type, eid=None):
        ...
        rslt = q.projection(sql, p, self.SOPTS)
        if len(rslt) > 0:
            if len(rslt[0]) > 0:
                return rslt[0][0].val
        return 0
    ...
    def findTag (self, name, desc=None):
        ...
        res = query_serv.findAllByQuery(sql, p, self.SOPTS)
        if len(res) > 0:
    ...
    def checkOmeName(self, ome_name, old_omeName=None):
        ...
        exps = query_serv.findAllByQuery(sql, p, self.SOPTS)
        if len(exps) > 0:
            return True
        else:
            return False
    ...
    def checkGroupName(self, name, old_name=None):
        ...
        grs = query_serv.findAllByQuery(sql, p, self.SOPTS)
        if len(grs) > 0:
            return True
        else:
            return False
    ...
    def saveAndReturnObject (self, obj):
        ...
        """
        u = self.getUpdateService()
        res = u.saveAndReturnObject(obj, self.SOPTS)
        res.unload()
    ...
    def saveAndReturnFile(self, binary, oFile_id):
        ...
        store = self.createRawFileStore()
        store.setFileId(oFile_id, self.SOPTS);
        pos = 0
        rlen = 0
    ...
    def createShare(self, host, blitz_id, image, message, members, enable, expiration=None):
        ...
        if len(image) > 0:
            ...
            items.extend(q.findAllByQuery(sql, p, self.SOPTS))
        ...
        if members is not None:
            ...
            ms = q.findAllByQuery(sql, p, self.SOPTS)
    ...
    def listLastImportedImages (self):
        ...
        f.limit = rint(10)
        p.theFilter = f
        for e in tm.getMostRecentObjects(['Image'], p, False, self.SOPTS)["Image"]:
            yield ImageWrapper(self, e)
    ...
    def listMostRecentShares (self):
        ...
        f.limit = rint(10)
        p.theFilter = f
        for e in tm.getMostRecentShareCommentLinks(p, self.SOPTS):
    ...
    def listMostRecentComments (self):
        ...
        f.limit = rint(10)
        p.theFilter = f
        for e in tm.getMostRecentAnnotationLinks(None, ['CommentAnnotation'], None, p, self.SOPTS):
    ...
    def listMostRecentTags (self):
        ...
        f.limit = rint(200)
        p.theFilter = f
        for e in tm.getMostRecentAnnotationLinks(None, ['TagAnnotation'], None, p, self.SOPTS):
    ...
    def getDataByPeriod (self, start, end, eid, otype=None, page=None):
        ...
        if otype is not None and otype in ("Image", "Dataset", "Project"):
            otype = otype.title()
            for e in tm.getByPeriod([otype], rtime(long(start)), rtime(long(end)), p, True, self.SOPTS)[otype]:
                wrapper = KNOWN_WRAPPERS.get(otype.title(), None)
                im_list.append(wrapper(self, e))
        else:
            res = tm.getByPeriod(['Image', 'Dataset', 'Project'], rtime(long(start)), rtime(long(end)), p, True, self.SOPTS)
            try:
                for e in res['Image']:
    ...
    def countDataByPeriod (self, start, end, eid, otype=None):
        ...
        p.theFilter = f
        if otype == 'image':
            return tm.countByPeriod(['Image'], rtime(long(start)), rtime(long(end)), p, self.SOPTS)['Image']
        elif otype == 'dataset':
            return tm.countByPeriod(['Dataset'], rtime(long(start)), rtime(long(end)), p, self.SOPTS)['Dataset']
        elif otype == 'project':
            return tm.countByPeriod(['Project'], rtime(long(start)), rtime(long(end)), p, self.SOPTS)['Project']
        else:
            c = tm.countByPeriod(['Image', 'Dataset', 'Project'], rtime(long(start)), rtime(long(end)), p, self.SOPTS)
    ...
    def getEventsByPeriod (self, start, end, eid):
        ...
        f.limit = rint(100000)
        try:
            f.groupId = rlong(self.SOPTS.getOmeroGroup())
        except:
            f.groupId = rlong(self.getEventContext().groupId)
        f.ownerId = rlong(eid or self.getEventContext().userId)
        p.theFilter = f
        return tm.getEventLogsByPeriod(rtime(start), rtime(end), p, omero.gateway.utils.ServiceOptsDict({'omero.group': -1}))
