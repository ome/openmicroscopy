...
def manage_experimenter(request, action, eid=None, conn=None, **kwargs):
    ...
    if action == 'new':
        ...
        initial={'omename': experimenter.userName, 'first_name':experimenter.firstName,
        ...
        if request.method != 'POST':
            ...
            name_check = conn.checkOmeName(request.REQUEST.get('omename'), experimenter.userName)
...
def my_account(request, action=None, conn=None, **kwargs):
    ...
    if action == "save":
        ...
        form = MyAccountForm(initial={'omename': experimenter.userName, 'first_name':experimenter.firstName,
                                    'middle_name':experimenter.middleName, 'last_name':experimenter.lastName,
                                    'email':experimenter.email, 'institution':experimenter.institution,
