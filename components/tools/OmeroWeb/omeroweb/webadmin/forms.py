...
class LoginForm(NonASCIIForm):
    ...
    ssl = forms.BooleanField(
        required=False, help_text='<img \
src="%swebgateway/img/nuvola_encrypted_grey16.png" title="Real-time encrypted \
data transfer can be turned on by checking the box, but it will slow down the \
data access. Turning it off does not affect the connection to the server \
which is always secure." alt="SSL"/>' % settings.STATIC_URL)
...
class GroupForm(NonASCIIForm):
    ...
    def __init__(self, name_check=False, group_is_current_or_system=False,
        ...
        self.fields['permissions'] = forms.ChoiceField(
            ...
            widget=forms.RadioSelect(), required=True, label="Permissions")
...
class MyAccountForm(NonASCIIForm):
    ...
    def __init__(self, email_check=False, *args, **kwargs):
        ...
        try:
            ...
            self.fields['default_group'] = GroupModelChoiceField(
                queryset=kwargs['initial']['groups'],
                initial=kwargs['initial']['default_group'], empty_label=None)
        except:
            self.fields['default_group'] = GroupModelChoiceField(
    ...
    omename = forms.CharField(
        ...
        widget=forms.TextInput(attrs={
            'onfocus': 'this.blur()', 'size': 30, 'autocomplete': 'off'}),
        label="Username")
    first_name = forms.CharField(
        max_length=250,
        widget=forms.TextInput(attrs={'size': 30, 'autocomplete': 'off'}))
...
class UploadPhotoForm(forms.Form):
    ...
    def clean_photo(self):
        if self.cleaned_data.get('photo') is None:
            raise forms.ValidationError(
                'No image selected. Supported image formats (file ' +
                'extensions allowed): jpeg, jpg, gif, png. The maximum ' +
                'image size allowed is 200KB.')
            ...
            try:
                if kwargs['initial']['entries']:
                    self.fields[str(e.id)] = forms.CharField(
                        max_length=250,
                        initial=e.value,
                        widget=forms.TextInput(attrs={'size': 30}), label=i+1)
                else:
                    self.fields[str(e.id)] = forms.CharField(
                        max_length=250,
                        widget=forms.TextInput(attrs={'size': 30}), label=i+1)
