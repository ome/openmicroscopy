...
var loadOtherPanels = function(inst, prefix) {
    ...
    if(selected.length > 0) {
        ...
        if (cm_var.content_details.rel!==null && cm_var.content_details.url!==null){
            $("div#content_details").html('<p>Loading data... please wait <img src ="/static/image/spinner.gif"/></p>');
...
function saveMetadata (image_id, metadata_type, metadata_value) {
    if (image_id == null) {
        ...
        $($('#id_'+metadata_type).parent()).append('<img src="/static/image/spinner.gif"/>');
        $.ajax({
            type: "POST",
