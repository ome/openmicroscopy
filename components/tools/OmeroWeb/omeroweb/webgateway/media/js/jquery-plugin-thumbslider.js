...
jQuery._ThumbSlider = function (container) {
  ...
  var _recalcSize = function () {
      ...
      _curspan() == 100 && slider.addClass('disabled') || slider.removeClass('disabled');
  };
  ...
  this.addThumb = function (elm, cb, error_cb) {
    var thumb = $(elm).appendTo(strip);
    _thumb_refcount++;
    var onload = function (fromerror) {
      ...
      if (_thumb_refcount == 0) {
        strip.children().fadeIn('fast', function() {_this.self.trigger('thumbsLoaded');});
        _this.center();
      }
      thumb.unbind('error', onerror);
    };
