...
To use this, create a file called C{seleniumtests.py} inside your app C{tests}
folder, and include the following template::
...
Of course you'll need to replace C{MyDjangoAppURLPrefix} with a real value,
and you can implement tests in any way you want, so as long as unittest can
run them.

By extending L{SeleniumTestBase} you get an instance variable names
C{selenium} which has the selenium RC client connected.

After all this is done, and assuming you have a standard C{Omero}
installation, you'll be able to issue:

  C{omero web seleniumtest MyDjangoApp seleniumserver.host \
http://omeroweb.host:port firefox}
...
class SeleniumTestServer (object):
    def __init__(self, base='webgateway', url='http://localhost:8000',
        ...
        if self.selenium is None:
            if self.host.find(':') >= 0:
                ...
                try:
                    ...
                    pass
            self.selenium = selenium(self.host, self.port, self.browser,
                                     self.url)
        self.selenium.start()
        self.selenium.open("/%s" % self.base)
        script = urllib2.urlopen("%s/static/3rdparty/jquery-1.11.1.js"
                                 % (self.url))
        r = script.read()
  ...
  if (inWindow.jQuery.expr[':'].containsExactly === undefined) {
    inWindow.jQuery.expr[':'].containsExactly = function (a,i,m) { return \
inWindow.jQuery(a).text() == m[3];};
  }
...
class SeleniumTestBase (unittest.TestCase):
    ...
    All tests will have the C{self.selenium} attr with a selenium client ready
    for usage.
    """
    SERVER = None
    ...
    def waitForElementPresence(self, element, present=True):
        ...
        for i in range(60):
            try:
                if self.selenium.is_element_present(element) == present:
                    break
            except:
                pass
            time.sleep(1)
    ...
    def waitForElementVisibility(self, element, visible):
        ...
        for i in range(60):
            try:
                if self.selenium.is_visible(element) == visible:
                    break
            except:
                if not visible:
                    break
