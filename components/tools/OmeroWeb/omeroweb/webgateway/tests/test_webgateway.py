...
class TestWebGatewayCacheTempFile(object):
    ...
    def testFilenameSize (self):
        ...
        try:
            ...
            raise
            pytest.fail('WebGatewayTempFile.new not handling special characters properly')
        ...
        try:
            ...
            pytest.fail('WebGatewayTempFile.new not handling long file names properly')
...
def testImageDataJson (gatewaywrapper, author_testimg):
    ...
    r = fakeRequest()
    v = views.imageData_json(r, iid=iid, server_id=1, conn=gatewaywrapper.gateway, _internal=True)
    assert type(v) == type('')
...
def testListChildrenJson (gatewaywrapper, author_testimg):
    ...
    r = fakeRequest()
    v = views.listImages_json(r, did=did, server_id=1, conn=gatewaywrapper.gateway, _internal=True)
    assert type(v) == type('')
