...
class TestHistory (object):
    ...
    def searchHistory(self, gateway, start, end, dtype="Dataset"):
        ...
        count = tm.countByPeriod([dtype], rtime(long(start)), rtime(long(end)), None, gateway.SERVICE_OPTS)
        data = tm.getByPeriod([dtype], rtime(long(start)), rtime(long(end)), None, True, gateway.SERVICE_OPTS)
        
        logs = tm.getEventLogsByPeriod(rtime(start), rtime(end), None, gateway.SERVICE_OPTS)
    ...
    def testCreateHistory(self, gatewaywrapper):
        ...
        gid = gatewaywrapper.gateway.createGroup("history-test-%s" % uuid, member_Ids=[userId], perms=READWRITE)
        ...
        switched = gatewaywrapper.gateway.c.sf.setSecurityContext(omero.model.ExperimenterGroupI(gid, False))
...
class TestScript (object):
    ...
    def setUp (self):
        """ Create a group with User """
        dbhelpers.USERS['script_test_user'] = dbhelpers.UserEntry('script_test_user',
                                                                  'ome',
                                                                  firstname='script',
                                                                  lastname='user',
                                                                  groupname="rw_script",
    ...
    def testRunScript(self, gatewaywrapper):
        ...
        # As Admin, create a second group with this user & upload script
        gatewaywrapper.loginAsAdmin()
        gid = gatewaywrapper.gateway.createGroup("script-test-%s" % uuid, member_Ids=[userId], perms=READWRITE)
        ...
        assert 'stdout' in results, "Failed to return stdout Original File. #8614"
