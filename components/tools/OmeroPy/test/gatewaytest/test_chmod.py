...
class ChmodBase (object):
    def doChange(self, gateway, group_id, permissions, test_should_pass=True, return_complete=True):
        """
    ...
    def assertCanEdit(self, blitzObject, expected=True,
        ...
        assert blitzObject.canEdit() ==  expected, "Unexpected result of canEdit(). Expected: %s" % expected
        gateway = blitzObject._conn
        # Now test if we can actually Edit and 'Hard link' the object
        nameEdited = False
        # for saves, omero group must *not* be -1
        origGroup = gateway.SERVICE_OPTS.getOmeroGroup()   # need to switch back to this after edits
        ...
        try:
            ...
            if isinstance(obj, omero.model.Image):
                ...
                rv = update.saveObject(link, gateway.SERVICE_OPTS)
        ...
        assert blitzObject.canEdit() ==  expected, "Unexpected result of canEdit(). Expected: %s" % expected
        assert nameEdited ==  expected, "Unexpected ability to Edit. Expected: %s" % expected
        assert objectUsed|sudo_needed ==  expected, "Unexpected ability to Use. Expected: %s" % expected
    ...
    def assertCanAnnotate(self, blitzObject, expected=True,
        ...
        assert blitzObject.canAnnotate() ==  expected, "Unexpected result of canAnnotate(). Expected: %s" % expected
        annotated = False
        try:
            ...
            if exc_info:
                traceback.print_exc()
        assert annotated ==  expected, "Unexpected ability to Annotate. Expected: %s" % expected
...
class TestChmodGroup (ChmodBase):
    ...
    def setUp (self):
        ...
        dbhelpers.USERS['chmod_group_admin'] = dbhelpers.UserEntry('r-_chmod_admin',
                                                                   'ome',
                                                                   firstname='chmod',
                                                                   lastname='admin',
                                                                   ...
                                                                   groupperms=READONLY,
                                                                   admin=True)
        dbhelpers.USERS['chmod_group_owner'] = dbhelpers.UserEntry('r-_chmod_owner',
                                                                   'ome',
                                                                   firstname='chmod',
                                                                   lastname='owner',
                                                                   groupname="ReadOnly_chmod_group",
    ...
    def testChmod(self, gatewaywrapper):
        ...
        dbhelpers.UserEntry.check_group_perms(gatewaywrapper.gateway, group_Name, READONLY)
        self.doChange(gatewaywrapper.gateway, group_Id, READWRITE)
        dbhelpers.UserEntry.check_group_perms(gatewaywrapper.gateway, group_Name, READWRITE)
...
class TestCustomUsers (ChmodBase):
    ...
    def testDelete8723(self, gatewaywrapper):
        ...
        assert p.canDelete() ==  True, "Member can delete another user's Project"
        handle = gatewaywrapper.gateway.deleteObjects("Project", [pr.id.val])
        cb = gatewaywrapper.gateway._waitOnCmd(handle)
...
class Test8800 (object):
    ...
    def testWithBlitzWrappers(self, author_testimg_generated):
        ...
        after = image.canEdit()
        assert before ==  after, "canEdit() affected by ImageWrapper.getPrimaryPixels()"
    ...
    def testWithoutWrappers(self, gatewaywrapper, author_testimg_generated):
        ...
        imgObj = gatewaywrapper.gateway.getContainerService().getImages("Image", (imgObj.id.val,), None, ctx)[0]
        after = imgObj.getDetails().getPermissions().canEdit()
        assert before ==  after, "canEdit() affected by loading image with 'omero.group':gid"

        ctx = {'omero.group':'-1'}
        imgObj = gatewaywrapper.gateway.getContainerService().getImages("Image", (imgObj.id.val,), None, ctx)[0]
        after = imgObj.getDetails().getPermissions().canEdit()
        assert before ==  after, "canEdit() should not be affected by 'omero.group':'-1'"
...
class TestDefaultSetup (object):
    ...
    def testAuthorCanEdit(self, gatewaywrapper, author_testimg_generated):
        ...
        gatewaywrapper.gateway.SERVICE_OPTS.setOmeroGroup('-1')
        i = gatewaywrapper.gateway.getObject("Image", imageId)
        assert None ==  i, "User cannot access Author's image in Read-only group"
        ...
        assert gatewaywrapper.gateway.setGroupForSession(image_gid)      # switch into group
        assert image_gid ==  gatewaywrapper.gateway.getEventContext().groupId, "Confirm in same group as image"
        i = gatewaywrapper.gateway.getObject("Image", imageId)
