...
class TestWrapper (object):
    ...
    def testAllObjectsWrapped (self, author_testimg):
        ...
        assert not hasattr(image.getArchived(), 'val'), "Shouldn't return rtype"
        assert not hasattr(image.getAcquisitionDate(), 'val'), "Shouldn't return rtype"
        assert not hasattr(image.sizeOfPixels(), 'val'), "Non 'get' methods shouldn't return rtype either"
        assert isinstance(image.getInstrument(), omero.gateway.InstrumentWrapper), "Should return InstrumentWrapper"
        assert isinstance(pixels, omero.gateway.BlitzObjectWrapper), "Should return a BlitzObjectWrapper"

        # 'get' methods should wrap model objects in BlitzObjectWrapper - allowing lazy loading
        assert isinstance(image.getFormat(), omero.gateway.BlitzObjectWrapper), "Should return a BlitzObjectWrapper"
        format = image.getFormat()
        assert format.value ==  "Deltavision", "BlitzObjectWrapper should lazy-load the value"
        ...
        assert isinstance(image.format, omero.model.FormatI), "Shouldn't wrap directly-accessed objects"
        ...
        assert isinstance(image.getPixels(0), omero.model.PixelsI), "Shouldn't wrap: No 'pixels' attribute"
    ...
    def testExperimenterWrapper (self, gatewaywrapper):
        gatewaywrapper.loginAsAdmin()
        e = gatewaywrapper.gateway.getObject("Experimenter", attributes={'omeName': gatewaywrapper.USER.name})
    ...
    def testDetailsWrapper (self, gatewaywrapper, author_testimg):
        ...
        assert d.getGroup().name ==  img.getProject().getDetails().getGroup().name
    ...
    def testSetters (self, gatewaywrapper):
        ...
        pytest.raises(AttributeError, getattr, self, 'something_wild_that_does_not_exist')
