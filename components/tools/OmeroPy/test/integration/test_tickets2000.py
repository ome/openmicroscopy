...
"""
   Integration tests for tickets between 1000 and 1999.
...
class TestTickets2000(lib.ITest):
    ...
    def test1064(self):
        self.client.sf.getShareService()
        self.client.sf.createSearchService()
        self.client.sf.getUpdateService()
    ...
    def test1071(self):
        ...
        assert 3 == len(hier), "len of hier != 3: %s" % [type(x)
                                                         for x in hier]
        for c in hier:
            if c.id.val == pr1.id.val and isinstance(c, ProjectI):
                assert c.sizeOfDatasetLinks() == 2,\
    ...
    def test1088(self):
        ...
        sql1 = "select el from EventLog el left outer join fetch el.event ev " \
               ...
               "and ev.id in (select id from Event where experimenter.id=:uid " \
               "and time > :start and time < :end)"

        sql2 = "select el from EventLog el left outer join fetch el.event ev " \
               "where el.entityType in ('ome.model.core.Pixels', 'ome.model.core.Image', " \
               "'ome.model.containers.Dataset', 'ome.model.containers.Project') " \
               "and ev.experimenter.id=:uid and ev.time > :start " \
               "and ev.time < :end"
            ...
            if m.id.val == exp.id.val:
                ...
                assert copied_id == got_id, \
                    ...
                    % (copied_id, got_id, exp.id.val,
                        [x.parent.id.val
                            for x in m.copyGroupExperimenterMap()],
    ...
    def test1184(self):
        ...
        sql = "select im from Image im join fetch im.details.owner " \
              ...
              "where d.id = :oid and im.details.owner.id=:eid order "\
              "by im.id asc"
