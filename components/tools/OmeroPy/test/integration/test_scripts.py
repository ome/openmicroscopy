...
class TestScripts(lib.ITest):
    ...
    def testDelete11371(self):
        ...
        for x in range(noOfScripts):
            uuid = self.uuid()
            f = self.pingfile()
            ids.append(svc.uploadOfficialScript("/%s/%s.py" %
                       (dirUuid, uuid), f.text()))
    ...
    def testRunScript(self):
        # Trying to run script as described in:
        # components/blitz/resources/omero/api/IScript.ice
        scriptService = self.root.sf.getScriptService()
        uuid = self.root.sf.getAdminService().getEventContext().sessionUuid
        client = self.root
        ...
        print userScriptId
        # scriptService.canRunScript(userScriptId)
        # returns 'True' here for some reason? (should be False)
        # But the method works in every other situation I have tried (Will).
        # Commenting out for now.
        # assert not scriptService.canRunScript(userScriptId)
        # ticket:2341
        results = {}
        try:
    ...
    def testEditScript(self):
        ...
        namedScripts = [s for s in scripts if s.path.val +
                        s.name.val == scriptPath]
...
client = s.client(
    "HelloWorld.py", "edited script", s.Long("a").inout(),
    s.String("b").inout())
...
"""
    ...
    def testScriptValidation(self):
        ...
        try:
            ...
            scriptService.uploadOfficialScript(
        ...
        assert - \
            1 == getId, "getScriptID() didn't return 'None' for invalid script"
        scripts = scriptService.getScripts()
        for s in scripts:
            assert s.mimetype.val == "text/x-python"
            assert s.path.val + \
    ...
    def testSpeedOfThumbnailFigure(self):
        ...
        process = svc.runScript(
            scriptID,
            wrap({"Data_Type": "Image", "IDs": [long(pixID)]}).val, None)
        ...
        # Script fails on hudson.
        # Only get returned Original Files (stderr, stdout)
        # but not the text of these.
