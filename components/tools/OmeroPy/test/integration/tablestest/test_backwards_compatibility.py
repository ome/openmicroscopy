...
class TestBackwardsCompatibility_5_10_3(BackwardsCompatibilityBase):

    def testCreateAllColumnsAndMetadata_5_10_3(self):
        """
        Call this method to create the reference HDF5 table under a 5.10.3
        Python 3.7 server. The OriginalFile ID of the table will be printed,
        and can be used to find the file under ${omero.data.dir}/Files/.
        Alternatively download it using
        ``omero download OriginalFile:FileID output.h5``
        ...
        To run manually goto ``components/tools/OmeroPy``, and run:
        ``pytest test/integration/tablestest/test_backwards_compatibility.py\
        -s -k testCreateAllColumnsAndMetadata_5_10_3``
        """
        grid = self.client.sf.sharedResources()
        repoMap = grid.repositories()
        repoObj = repoMap.descriptions[0]
    ...
    def testAllColumnsAndMetadata_5_10_3(self):
        """
        Check whether a table created under 5.10.3 is still usable with a newer
        server
        """
        ofile = self.uploadHdf5("service-reference-dev_5_10_3.h5.bz2")
