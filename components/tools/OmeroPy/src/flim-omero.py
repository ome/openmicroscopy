...
This script performs FRET analysis on FLIM images calculating Decay curve
differences between control and experimental datasets.
...
# eps (epsilon) is a magic small number to avoid getting division by zero
# warnings from the mpfit calls
eps = 0.000000001

# mycmdata1 and cycm1 are custom colormaps for generating plots in matplotlib
# of the numpy arrays of the spatial parameter distributions (the colours are
# chosen to make nice inverted images for presentations rather than nice look
# nice in their uninverted form.
...
paranames = ['$\\alpha_{1}$', '$\\tau_{1}$',
             '$b$', '$\\alpha_{2}$', '$\\tau_{2}$']

# A functions to locate the index of the maximum value in an array
argmax = lambda a: max(itr.izip(a, xrange(len(a))))[1]

# The following functions are the single exponential and the double exponential
# functions and the wrapper functions for the mpfit program.

# Single exponential decay function convolved with irf
cexp_1 = lambda p, x, i: npy.convolve(
    p[0] * npy.e ** (-x / (p[1] + eps)) + p[2], i[::-1], mode=2)

# Single exponential decay function wrapped for calling by mpfit


def err_1(p, fjac=None, x=None, y=None,
          err=None, i=None, sh=None, sm=None, lf=None):
    err = None
    model = cexp_1(p, x, i)
    status = 0
    if err is None:
...
cexp_2 = lambda p, x, i: npy.convolve(
    p[0] * npy.e ** (-x / (p[1] + eps))
    + p[3] * npy.e ** (-x / (p[4] + eps))
    + p[2], i[::-1], mode=2)
...
def err_2(p, fjac=None, x=None, y=None,
          err=None, i=None, sh=None, sm=None, lf=None):
    err = None
    model = cexp_2(p, x, i)
    status = 0
...
# This function fits a numpy array s representing the signal and e
# respresenting the variance against a model given by mode over time points
# given by a_tp. It returns a large number of values so that the global average
...
def procFitIt(s, e, a_tp, mode, d_args, par=None):
    ...
    i_slice = len(a_tp)
    # create 2n long arrays for convolution of model with instrument response
    # fucntion
    ...
    if mode > 1:
        # fit double exponential (FRET condition fit) if there are no initial
        # parameters generate them
        # THESE ARE MAGIC NUMBER DEFAULTS FOR INITAL VALUES
        # At this point for average initial fit calculate the shift to align
        # the peak of the convolved model signal with the peak of the actual
        # signal
        if par is None:
            par = npy.array(
                [1.0,  d_args['n'] / 2.0, 0.0, 1.0,  d_args['n']], 'd')
        ...
        fa = {'x': tp, 'y': sig, 'i': irf, 'sh': shift,
              'sm': smax, 'err': se, 'lf': i_slice}
        #  DO THE FIT
    ...
    # Here a lot of values are returned so that higher level calls can lot the
    # fit
    fpar = []
    ...
    fpar.append(smax)
    fpar.append(shift)
    return [fit, tp[0:i_slice], cf1[shift:shift + i_slice],
            sig[0:i_slice], se[0:i_slice], npy.array(fpar)]

# This routine performs a k-means segmentation of the average projection of the
# data to identify the focal pixels it is passed the whole file as rawdata and
# the parameter dictionary so it can know the structure of the data spatial
# dimensions , time points. it also makes an initial fit of the global averaged
# data for focal pixels and hence needs to know the model to use
...
def procSEG(rawdata, a_tp, d_args):
    ...
    # find cell boundary use stdev and average projections for K-means
    # thresholding on normalized image stacks
    ...
    # At this point i would be possible to segment the images further with
    # call scipy watershedding and labeling to generate separate regions of
    # interest for each cell with in the image. However this would have knock
    # on effect on distribution of pixels in mpi. Also watershedding might
    # need manual tuning to prevent shattering cells Calculate average focal
    # pixel intensities and variance
    ...
    initfit, tp, cf1, sig, se, fpar = procFitIt(
        zacel, s1cel, a_tp, i_mode, d_args, None)
    return [ocel.reshape((i_res, i_res)), initfit, tp, cf1, sig, se, fpar]

# This function reads the whole file of pixels in for each node in mpi
# cluster and returns a numpy array
...
def procRead(rawPixelsStore, pixels):
    ...
    # rawdata = npy.fromfile(
    #     file=fd, dtype=npy.uint16).byteswap().reshape(shape)
    # return rawdata
...
def procTP(rawFileStore, queryService, instrumentResponseFileId):
    return script_utils.readFileAsArray(rawFileStore, queryService,
                                        instrumentResponseFileId, 256, 2,
                                        separator=' ')
...
def getParentProject(containerService, datasetId):
    projectList = containerService.findContainerHierarchies(
        'Project', [datasetId], None, None)
    return projectList[0]


def getDataset(containerService, datasetId):
    datasetList = containerService.loadContainerHierarchy(
        'Dataset', [datasetId], None, None)
    return datasetList[0]


def getProject(containerService, projectId):
    projectList = containerService.loadContainerHierarchy(
...
def attachFileToDataset(containerService, queryService,
                        updateService, rawFileStore, datasetID, localName):
    dataset = getDataset(containerService, datasetID)
    return script_utils.uploadAndAttachFile(queryService, updateService,
                                            rawFileStore, dataset, localName,
                                            'PDF', description=None,
                                            namespace=None,
                                            origFilePathName=None)


def attachFileToProject(containerService, queryService,
                        updateService, rawFileStore, projectId, localName):
    project = getProject(containerService, projectId)
    return script_utils.uploadAndAttachFile(queryService, updateService,
                                            rawFileStore, project, localName,
                                            'PDF', description=None,
                                            namespace=None,
                                            origFilePathName=None)
...
def mpi_run(argv):
    ...
    # Process of rank=0 acts as co-ordinating process and does the latex
    # generation
    comm = mpi.COMM_WORLD
    size = comm.Get_size()
    rank = comm.Get_rank()
    ...
    try:
        opts, args = gop.getopt(argv, "h:u:p:c:e:r:o",
                                ["host", "user", "password", "control",
                                 "experiment", "rfid", "outputfilename"])
    ...
    client, session = createSession(
        d_args['host'], d_args['user'], d_args['password'])
    ...
    a_rawfiles.append(
        imagesInDataset(containerService, int(d_args['control'])))
    a_rawfiles.append(
        imagesInDataset(containerService, int(d_args['experiment'])))
    ...
    while not done:
        ...
        # set up meta parameter dictionary that contains useful parameters for
        # subroutines
        # m model type 1 or 2
        # f file name
        d_args['i'] = s_id
        ...
        # every process gets a copy of the whole rawdata at for current file
        # at this point this would better if only process of rank 0 did this
        # and then once pixels have been assigned scattered to each process
        # they got just the pixels they need but this works for now
        localraw = procRead(rawPixelsStore, d_args['f'])
        localpix = []
        # This is where process rank=0 segments the image and performs global
        # average fit then generates pixel lists that are to be scattered to
        # each of the processed so each process knows which pixels it is
        # working on
        if rank == 0:
            tex.write("\\verb=%s= \\\\ \n" % d_args['i'])
            start = mpi.Wtime()
            celpix, initfit, tp, cf1, sig, se, ipar = procSEG(
                localraw, a_tp, d_args)
            ...
            for r in range(size):
                pix = [
                    r + size * i
                    for i in range(int(float(len(pixarray)) / size) + 1)
                    if r + size * i < len(pixarray)]
                localpix.append([])
                for p in pix:
        ...
        # Make the local grids for the processes to fill in the parameter
        # values they calculate Again here each process has full size grid but
        # only fill in the pixels it has been assigned
        ...
        for idx in localpixels:
            ...
            if (idx[0] > 0 and idx[0] < i_sres
                    and idx[1] > 0 and idx[1] < i_sres):
                s = localraw.reshape((i_slice, i_sres, i_sres))[
                    :, idx[0] - 1:idx[0] + 1,
                    idx[1] - 1:idx[1] + 1].mean(axis=1).mean(axis=1)
                e = localraw.reshape((i_slice, i_sres, i_sres))[
                    :, idx[0] - 1:idx[0] + 1,
                    idx[1] - 1:idx[1] + 1].std(axis=1).std(axis=1)
                e = npy.zeros(len(s)) + 1
                fit, tp, cf1, sig, se, fpar = procFitIt(
                    s, e, a_tp, i_mode, d_args, localparams)
                totalfitcalls += 1
                c[idx[0], idx[1]] = fit.fnorm
                g[idx[0], idx[1]] = fit.status
                if i_mode > 1:
                    a1[idx[0], idx[1]] = fit.params[
                        0] / (fit.params[0] + fit.params[3])
                    k1[idx[0], idx[1]] = fit.params[1]
                    b[idx[0], idx[1]] = fit.params[2]
                    a2[idx[0], idx[1]] = fit.params[
                        3] / (fit.params[0] + fit.params[3])
                    k2[idx[0], idx[1]] = fit.params[4]
                    f[idx[0], idx[1]] = (
                        1.0 - k1[idx[0], idx[1]] / k2[idx[0], idx[1]])
                else:
        ...
        # Once process rank=0 has gathered all the grids of pixels from the
        # other processes collate them to produce one grid with all the values
        # in
        if rank == 0:
            ...
            for i in range(len(rg)):
                ...
                if i_mode > 1:
                    ga2 += npy.array(ra2[i])
                    gk2 += npy.array(rk2[i])
            # AT THIS POINT NEED TO CALCULATE DISTRIBUTION AND SELECT
            # NO-FRET LIFE TIME FOR GROUP1
            ...
            fig.clf()
            # This is where process rank=0 saves the csv files with the
            # parameters

            npy.savetxt(
                s_dir + 'out/gg_' + str(s_id) + '.txt',
                gg, fmt='%10.5f', delimiter=',')
            npy.savetxt(
                s_dir + 'out/ga1_' + str(s_id) + '.txt',
                ga1, fmt='%10.5f', delimiter=',')
            npy.savetxt(
                s_dir + 'out/gk1_' + str(s_id) + '.txt',
                gk1, fmt='%10.5f', delimiter=',')
            npy.savetxt(
                s_dir + 'out/gf_' + str(s_id) + '.txt',
                gf, fmt='%10.5f', delimiter=',')
            npy.savetxt(
                s_dir + 'out/gb_' + str(s_id) + '.txt',
                gb, fmt='%10.5f', delimiter=',')
            npy.savetxt(
            ...
            tex.write(
                "\\includegraphics[scale=0.3]{%sout/gk1h_%s.pdf}& \n"
                % (s_dir, str(s_id)))
            tex.write(
                "\\includegraphics[scale=0.3]{%sout/fit_%s.pdf}\\\\ \n"
                % (s_dir, str(s_id)))
            ...
            tex.write(
                "\\includegraphics[scale=0.2]{%sout/ga1_%s.png}& \n"
                % (s_dir, str(s_id)))
            tex.write(
                "\\includegraphics[scale=0.2]{%sout/gk1_%s.png}& \n"
                % (s_dir, str(s_id)))
            tex.write(
                "\\includegraphics[scale=0.2]{%sout/gb_%s.png}\\\\ \n"
                % (s_dir, str(s_id)))
            tex.write('$\\alpha_{1}$&$\\tau_{1}$&$b$\\\\ \n')
            if i_mode > 1:
                tex.write(
                    "\\includegraphics[scale=0.2]{%sout/ga2_%s.png}& \n"
                    % (s_dir, str(s_id)))
                tex.write(
                    "\\includegraphics[scale=0.2]{%sout/gk2_%s.png}& \n"
                    % (s_dir, str(s_id)))
                tex.write(
                    "\\includegraphics[scale=0.2]{%sout/gf_%s.png}\\\\ \n"
                    % (s_dir, str(s_id)))
                tex.write('$\\alpha_{2}$&$\\tau_{2}$&$E_{f}$\\\\ \n')
            tex.write(
                "\\includegraphics[scale=0.2]{%sout/gc_%s.png}& \n"
                % (s_dir, str(s_id)))
            tex.write(
                "\\includegraphics[scale=0.2]{%sout/gg_%s.png}\\\\ \n"
                % (s_dir, str(s_id)))
            tex.write('$\\chi^{2}$&$fit$\\\\ \n')
            tex.write("\\end{tabular} \n")
            tex.write("\\end{figure} \n")
        ...
        # This line syncs the processes so they wait for process rank=0 to do
        # its collating and output before getting next file
        comm.Allreduce(
            [npy.asarray(totalfitcalls, 'i'), mpi.INT],
            [ncalls, mpi.INT], op=mpi.SUM)
        if rank == 0:
            tex.write('Total pixels fitted=%d\\\\ \n' % (ncalls))
            tex.flush()
        fcount += 1
        # Here the cumlative mean no-fret life time is calculated and the group
        # is changed once all the no-fret files have been processed
    ...
    if rank == 0:
        ...
        tex.close()
        os.system(
            'pdflatex -output-directory=' + s_dir + ' ' + s_dir + texfilename)
        pdfFile = texfilename.rsplit('.tex')[0] + '.pdf'
        attachFileToProject(
            containerService, queryService,  updateService,
            rawFileStore, getParentProject(
                containerService,
                int(d_args['experiment'])).getId().getValue(),
            s_dir + pdfFile)
        # -------------------------
