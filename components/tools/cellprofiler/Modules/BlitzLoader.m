function handles = BlitzLoader(handles)
...
if SetBeingAnalyzed == 1
    ...
    cnt = 0;
    for i = 1:list.size()
        pixelsList = blitzGateway.getPixelsFromImage(list.get(i-1).id.val);
    ...
    for i = 1:length(fileIds),
        pixels = getPixelsOMERO(blitzGateway, fileIds(i));
...
for n = 1:handles.Pipeline.imagesPerSet
        ...
        try
            ...
            [LoadedImage, handles] = CPOMEROimread(blitzGateway, currentFileDetails, TextToFind{n}, handles);
            ...
            [path, fname, ext, v] = fileparts(getFileName(blitzGateway, imageId));
       ...
       FileNames{n} = {strcat(getFileName(blitzGateway, imageId), ':', num2str(z), ':', num2str(t))};
end
if (~blitzGateway.isClosed())
    blitzGateway.close();
end
