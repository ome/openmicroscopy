...
public class URoiTest extends TestCase {
     ...
     * -------------------------------------------------------- create table
     * CellLineage2 (type varchar, slice int, time int, x1 real, y1 real, x2
     * real, y2 real, text varchar, color char(6), filled boolean, grouped
     * varchar, Notes varchar); copy CellLineage2 from '/tmp/cell2' with null as
     * 'N/A'; --------------------------------------------------------
     */
    protected Region cellLineage(final Pixels pix) {
        ...
        ParameterizedRowMapper<Overlay> prm = new ParameterizedRowMapper<Overlay>() {
        ...
        List<Overlay> list = jdbc.query(
                "select * from CellLineage2 where time = 1", prm);
