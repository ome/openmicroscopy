...
public class OMEROWrapper extends MinMaxCalculator {
    ...
    public OMEROWrapper() {
        try {
            iReader = new ImageReader(new ClassList("readers.txt",
                    IFormatReader.class));
     ...
     *            The path to the file.
     * @throws FormatException
     *             if there is an error parsing metadata.
     * @throws IOException
     *             if there is an error reading the file.
     */
    public void setChannelGlobalMinMax(String id) throws FormatException,
            IOException {
        for (int c = 0; c < getSizeC(); c++) {
            ...
            getMetadataStore().setChannelGlobalMinMax(c, gMin, gMax,
                    new Integer(getSeries()));
        }
    }
     ...
     *             if there is an error reading the file.
     */
    public void finalizeMetadataStore(String id) throws FormatException,
            IOException {
    ...
    public boolean isMinMaxSet() throws FormatException, IOException {
        if (minMaxSet == null) {
            OMEROMetadataStore store = (OMEROMetadataStore) reader
                    .getMetadataStore();
            List<Pixels> p = (ArrayList<Pixels>) store.getRoot();
            int series = reader.getSeries();
            if (p.get(series).getChannel(getSizeC() - 1).getStatsInfo() == null) {
                minMaxSet = false;
            } else {
                minMaxSet = true;
            }
    ...
    public void populateMinMax(Long id, Integer i) throws FormatException,
            IOException {
        if (isMinMaxSet() == false) {
            OMEROMetadataStore store = (OMEROMetadataStore) reader
                    .getMetadataStore();
