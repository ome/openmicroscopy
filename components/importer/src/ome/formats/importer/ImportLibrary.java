...
public class ImportLibrary {
    ...
    private final OMEROMetadataStore store;

    private final OMEROWrapper reader;

    private final ImportContainer[] fads;
            ...
            for (int t = 0; t < sizeT; t++) {
                for (int c = 0; c < sizeC; c++) {
                    for (int z = 0; z < sizeZ; z++) {
                        ...
                        ByteBuffer buf = reader.openPlane2D(fileName,
                                planeNumber, arrayBuf).getData();
     ...
     * @throws FormatException
     */
    public boolean isLittleEndian(String fileName) throws FormatException,
            IOException {
    ...
    private byte[] swapIfRequired(ByteBuffer buffer, String fileName)
        ...
        // We've got nothing to do if the samples are only 8-bits wide or if
        // they
        // are floating point.
        ...
        // System.err.println(fileName + " is Little Endian: " +
        // isLittleEndian(fileName));
        if (isLittleEndian(fileName)) {
            if (bytesPerPixel == 2) { // short
                ...
                throw new FormatException("Unsupported sample bit width: '"
                        + bytesPerPixel + "'");
            }
        }
