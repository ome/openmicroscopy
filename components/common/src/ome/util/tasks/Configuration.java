 ...
 * {@link ServiceFactory} and {@link Task} objects if given the proper arguments.
 * 
 ...
 * </ul>
 * 
 * To login as a root, for example, {@link Properties} of the form:
 * <code>
...
public class Configuration
{
	...
	/**
	 * Enumeration of the string values which will be used directly by {@link Configuration}.
	 */
	public enum Keys {
	...
	/** Sole constructor. Performs the necessary parsing of the {@link Properties}
	 * values.
	 * 
	 * @param props Not null.
	 */
	public Configuration(Properties props) 
	{
		...
		if (taskClass == null)
			throw new IllegalArgumentException(
					"Cannot find task class for:"+task);
		...
		if (p(user) != null || p(group) != null || p(type) != null || p(pass) != null)
		{
			if (p(user) == null || p(group) == null || p(type) == null || p(pass) == null)
			{
				throw new IllegalArgumentException(
	...
	/** Returns a non-null {@link Server} instance only if server arguments
	 * were provided in the {@link Properties} 
	 */
	public Server getServer() {
		return server;
	}

	/** Returns a non-null {@link Login} instance only if login arguments
	 * were provided in the {@link Properties} 
	 */
	...
	/** Creates a new {@link Task} based on the values of {@link #getProperties()},
	 * {@link #getTaskClass()}, and {@link #createServiceFactory()}.
	 * 
	 * @return a non-null {@link Task} instance, ready for execution.
	 * @throws RuntimeException if anything happens during the reflection-based
	 *         creation of the {@link Task}
	 */
	public Task createTask() {
		Constructor<Task> ctor;
		try {
			ctor = getTaskClass().getConstructor(ServiceFactory.class, Properties.class);
			Task newTask = ctor.newInstance(createServiceFactory(),getProperties());
			return newTask;
		} catch (Exception e) {
	...
	/** 
	 * Returns the property for the {@link Keys#toString()} value of the argument. 
	 */
