...
public class ETable extends JTable {
    ...
    private static final Color MAC_UNFOCUSED_SELECTED_CELL_HORIZONTAL_LINE_COLOR = new Color(
            0xe0e0e0);

    private static final Color MAC_UNFOCUSED_SELECTED_CELL_BACKGROUND_COLOR = new Color(
            0xc0c0c0);

    private static final Color MAC_FOCUSED_UNSELECTED_VERTICAL_LINE_COLOR = new Color(
            0xd9d9d9);

    private static final Color MAC_FOCUSED_SELECTED_VERTICAL_LINE_COLOR = new Color(
            0x346dbe);

    private static final Color MAC_UNFOCUSED_UNSELECTED_VERTICAL_LINE_COLOR = new Color(
            0xd9d9d9);

    private static final Color MAC_UNFOCUSED_SELECTED_VERTICAL_LINE_COLOR = new Color(
            0xacacac);

    /**
     * The background color for alternate rows in lists and tables.
     */
    public static final Color ALTERNATE_ROW_COLOR = new Color(0.92f, 0.95f,
            0.99f);

    public ETable() {
        // Although it's the JTable default, most systems' tables don't draw a
        // grid by default.
        ...
        // The Aqua and GTK LAFs ignore the grid settings anyway, so this causes
        // no change there.
        setShowGrid(false);

        // Tighten the cells up, and enable the manual painting of the vertical
        // grid lines.
        ...
        try {
            java.lang.reflect.Method setAutoCreateRowSorterMethod = JTable.class
                    .getDeclaredMethod("setAutoCreateRowSorter",
                            new Class[] { boolean.class });
            setAutoCreateRowSorterMethod.invoke(this, true);
        } catch (Exception ex) {
            // Ignore. Likely we're on Java 5, where this functionality doesn't
            // exist.
        }
    }
    ...
    /**
     * Tests whether we're running on Mac OS. The Mac is quite different from
     * Linux and Windows, and it's sometimes necessary to put in special-case
     * behavior if you're running on the Mac.
     */
    ...
    /**
     * Tests whether we're using the GTK+ LAF (and so are probably on Linux or
     * Solaris).
     */
    ...
    protected void paintEmptyRows(Graphics g) {
        ...
        if (rowCount * rowHeight < height) {
            ...
            // Mac OS' Aqua LAF never draws vertical grid lines, so we have to
            // draw them ourselves.
    ...
    public Component prepareRenderer(TableCellRenderer renderer, int row,
        ...
        if (selected) {
            if (isMacOs() && focused == false) {
                // Native Mac OS renders the selection differently if the table
            ...
            // Outside of selected rows, we want to alternate the background
            // color.
        ...
        if (c instanceof JComponent) {
            ...
            // The Java 6 GTK LAF JCheckBox doesn't paint its background by
            // default.
            // Sun 5043225 says this is the intended behavior, though presumably
            // not when it's being used as a table cell renderer.
            ...
            if (getCellSelectionEnabled() == false && isEditing() == false) {
                if (isMacOs()) {
                    ...
                    // It does however draw a horizontal line under the whole
                    // row, and a vertical line separating each column.
    ...
    private void fixMacOsCellRendererBorder(JComponent renderer,
        ...
        if (getShowVerticalLines()) {
            ...
            if (focused) {
                verticalLineColor = selected ? MAC_FOCUSED_SELECTED_VERTICAL_LINE_COLOR
                        : MAC_FOCUSED_UNSELECTED_VERTICAL_LINE_COLOR;
            } else {
                verticalLineColor = selected ? MAC_UNFOCUSED_SELECTED_VERTICAL_LINE_COLOR
                        : MAC_UNFOCUSED_UNSELECTED_VERTICAL_LINE_COLOR;
            }
            Border verticalBorder = BorderFactory.createMatteBorder(0, 0, 0, 1,
                    verticalLineColor);
    ...
    public Point getToolTipLocation(MouseEvent e) {
        ...
        // cells without tool tips will show an empty tool tip until the tool
        // tip mode times out (or the table has a global default tool tip).
        // (ToolTipManager.checkForTipChange considers a non-null result from
        // getToolTipText *or* a non-null result from getToolTipLocation as
        // implying that the tool tip should be displayed. This seems like a
        // bug, but that's the way it is.)
    ...
    protected void configureEnclosingScrollPane() {
        ...
        if (p instanceof JViewport) {
            ...
            if (gp instanceof JScrollPane) {
                ...
                final Component renderer = new JTableHeader()
