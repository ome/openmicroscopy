...
public class ThumbnailServiceTest extends TestCase {
    ...
    private final static Log log = LogFactory
            .getLog(ThumbnailServiceTest.class);
    ...
    public void testThumbnailsDirect() throws Exception {
        ...
        // fails RenderingDef def =
        // sf.getPixelsService().retrieveRndSettings(p.getId());
        RenderingDef def = sf.getQueryService().findByQuery(
                "from RenderingDef r where r.pixels.id = :pix_id and "
                        + "r.details.owner.id = :owner_id",
                new Parameters().addLong("pix_id", p.getId()).addLong(
                        "owner_id", 0L));
        // WON'T COMPILE sf.createThumbnailService().getThumbnailDirect(p, def,
        // 8, 8);
