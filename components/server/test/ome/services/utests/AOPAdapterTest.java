...
public class AOPAdapterTest extends MockObjectTestCase {
    ...
    static {
        System.getProperties().setProperty("container.status",
                "out-of-container");
    }
    ...
    public void testOwnContext() throws Throwable {
        ...
        // the interceptor
        BeanDefinition handlerDef = new RootBeanDefinition(
                SimpleInterceptor.class, null, null);
        context.registerBeanDefinition("handler", handlerDef);
        // the proxy factory
        MutablePropertyValues mpv = new MutablePropertyValues();
        mpv.addPropertyValue("interceptorNames", Collections
                .singleton("handler"));
        mpv.addPropertyValue("proxyInterfaces", Collections
                .singleton(IBean.class.getName()));
        mpv.addPropertyValue("target", new RuntimeBeanReference("bean"));
        BeanDefinition factoryDef = new RootBeanDefinition(
                ProxyFactoryBean.class, null, mpv);
        ...
        mockInvocation.expects(atLeastOnce()).method("getMethod").will(
                returnValue(method));
        mockInvocation.expects(atLeastOnce()).method("getParameters").will(
                returnValue(new Object[] {}));
        mockInvocation.expects(atLeastOnce()).method("proceed").will(
                returnValue(null));
        ...
        // checks
        assertTrue("This handler should get called.",
