...
public class LoadContainersQueryTest extends AbstractManagedContextTest {
    ...
    protected void creation_fails(Parameters parameters) {
        try {
            q = new PojosLoadHierarchyQueryDefinition( // TODO if use lookup,
                                                        // more generic
                    parameters);
    ...
    public void test_simple_usage() throws Exception {
        Long doesntExist = -1L;
        q = new PojosLoadHierarchyQueryDefinition(new Parameters().addIds(
    ...
    public void test_owner_filter_user_obj() throws Exception {
        ...
        q = new PojosLoadHierarchyQueryDefinition(new Parameters(ids).addAll(
    ...
    public void test_leaves_handled_properly() throws Exception {
        PojoOptions withLeaves = new PojoOptions().leaves(), noLeaves = new PojoOptions()
                .noLeaves();

        runLevel(Project.class, Arrays.asList(DATA.pu9992.getId(), DATA.pu9991
                .getId()), withLeaves);
        ...
        runLevel(Dataset.class, Arrays.asList(DATA.du7772.getId(), DATA.du7771
                .getId()), withLeaves);
    ...
    private void runLevel(Class klass, List ids, PojoOptions po) {
        q = new PojosLoadHierarchyQueryDefinition(new Parameters().addIds(ids)
                .addOptions(po.map()).addClass(klass));
    ...
    private void check_pd_ids(List ids) {
        for (Project prj : (List<Project>) list) {
            ...
            assertTrue("And our datasets weren't there", datasetIds
                    .containsAll(ids));
        }
    }
    ...
    private void run_null_filter_check_size(Class klass, int size) {
        q = new PojosLoadHierarchyQueryDefinition(new Parameters(filterForUser)
                .addClass(klass).addIds(null));

        list = (List) iQuery.execute(q);
        assertTrue(String.format(
                "Didn't find all our objects of type %s, %d < %d ", klass
                        .getName(), list.size(), size), list.size() >= size);
    }
