...
public class OmeroSessionFactoryBean extends LocalSessionFactoryBean {
     ...
     * "example.hbm.xml" or "mypackage/example.hbm.xml". Resources stored as
     * <b>cached</b>MappingResouces will use the Configuration.addCacheableFile
     * call.
     * 
    ...
    public void setCachedMappingResources(String[] mappingResources) {
        ...
        for (int i = 0; i < mappingResources.length; i++) {
            this.cachedMappingResources[i] = new ClassPathResource(
    ...
    protected void postProcessConfiguration(Configuration config)
        ...
        if (this.cachedMappingResources != null) {
            for (int i = 0; i < this.cachedMappingResources.length; i++) {
                try {
                    config.addCacheableFile(this.cachedMappingResources[i]
                            .getFile());
                } catch (Exception e) {
                    throw new RuntimeException(
                            "Failed to load Hibernate mapping resource "
                                    + this.cachedMappingResources[i], e);
                }
            }
        }
