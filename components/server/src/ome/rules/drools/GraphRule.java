...
/**
 * walks a graph of objects {@see ome.util.ContextFilter} and asserts all
...
public class GraphRule {
    ...
    @Condition
    public boolean notYetParsed(@Data("cache")
    Set dataCache, @Fact("node")
    Filterable node) {
        return !dataCache.contains(node);
    }

    @Consequence
    public void filter(@Data("cache")
    Set dataCache, @Fact("node")
    Filterable node, KnowledgeHelper kh) {
