/*
 * org.openmicroscopy.omero.dao.utils.FormalOutputUtils
 *
 *------------------------------------------------------------------------------
 *
...
import org.openmicroscopy.omero.model.FormalOutput;
...
public class FormalOutputUtils  extends BaseModelUtils {

  private static Log log = LogFactory.getLog(FormalOutputUtils.class);
  ...
  public void clean(Object o, Set done){
    ...
    FormalOutput self = (FormalOutput) o;
    ...
    if (null==self.getModule()){
      ...
      self.setModule(null);
         if (log.isDebugEnabled()){
             log.debug("Set FormalOutput.module to null");
         }
    } else {
      (new org.openmicroscopy.omero.model.Module()).getUtils().clean(self.getModule(),done);
    }
	// Type: org.hibernate.type.SetType(org.openmicroscopy.omero.model.FormalOutput.analysisChainLinks)
    // Cleaning java.util.Set::analysisChainLinks set (Role: org.openmicroscopy.omero.model.FormalOutput.analysisChainLinks)
    if (null==self.getAnalysisChainLinks()){
      ...
      self.setAnalysisChainLinks(null);
         if (log.isDebugEnabled()){
             log.debug("Set FormalOutput.analysisChainLinks to null");
         }
    } else {
    ...
    if (null==self.getSemanticType()){
      ...
      self.setSemanticType(null);
         if (log.isDebugEnabled()){
             log.debug("Set FormalOutput.semanticType to null");
         }
    } else {
