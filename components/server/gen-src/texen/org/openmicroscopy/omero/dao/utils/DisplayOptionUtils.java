/*
 * org.openmicroscopy.omero.dao.utils.DisplayOptionUtils
 *
 *------------------------------------------------------------------------------
 *
...
import org.openmicroscopy.omero.model.DisplayOption;
...
public class DisplayOptionUtils  extends BaseModelUtils {

  private static Log log = LogFactory.getLog(DisplayOptionUtils.class);
  ...
  public void clean(Object o, Set done){
    ...
    DisplayOption self = (DisplayOption) o;
    ...
    if (null==self.getImage()){
      ...
      self.setImage(null);
         if (log.isDebugEnabled()){
             log.debug("Set DisplayOption.image to null");
         }
    } else {
    ...
    if (null==self.getImagePixel()){
      ...
      self.setImagePixel(null);
         if (log.isDebugEnabled()){
             log.debug("Set DisplayOption.imagePixel to null");
         }
    } else {
    ...
    if (null==self.getModuleExecution()){
      ...
      self.setModuleExecution(null);
         if (log.isDebugEnabled()){
             log.debug("Set DisplayOption.moduleExecution to null");
         }
    } else {
      (new org.openmicroscopy.omero.model.ModuleExecution()).getUtils().clean(self.getModuleExecution(),done);
    }
	// Type: org.hibernate.type.SetType(org.openmicroscopy.omero.model.DisplayOption.displayRois)
    // Cleaning java.util.Set::displayRois set (Role: org.openmicroscopy.omero.model.DisplayOption.displayRois)
    if (null==self.getDisplayRois()){
      // Do nothing
    } else if (!Hibernate.isInitialized(self.getDisplayRois())){
      self.setDisplayRois(null);
         if (log.isDebugEnabled()){
             log.debug("Set DisplayOption.displayRois to null");
         }
    } else {
