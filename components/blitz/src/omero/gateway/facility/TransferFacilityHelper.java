...
public class TransferFacilityHelper {
    ...
    public TransferFacilityHelper(Gateway gateway, DataManagerFacility datamanager, TransferFacility parent)
    ...
    Object importFile(ImportableObject object,
            ImportableFile importable, ExperimenterData exp, boolean close) 
            throws ImportException, DSAccessException, DSOutOfServiceException
        {
            ...
            if (!CollectionUtils.isEmpty(tags)) {
                ...
                try {
                    if (l.size() > 0) {
                        l = datamanager.saveAndReturnObject(ctx, l, parameters, userName);
                    }
            ...
            if (file.isFile()) {
                ...
                if (ImportableObject.isArbitraryFile(file)) {
                    ...
                    if (size == 0) {
                        ...
                        if (status.isMarkedAsCancel())
                            return Boolean.valueOf(false);
                        return importImageFile(ctx, object, ioContainer,
                                importIc, status, close, userName);
                    } else {
                        List<ImportContainer> containers = ic.getContainers();
                        hcs = isHCS(containers);
                        Map<File, ImportCallback> files = 
                            new HashMap<File, ImportCallback>();
            ...
            if (hcsFiles.size() > 0) {
                if (container != null && container instanceof ScreenData) {
                    ...
                    } else ioContainer = browse.findIObject(ctx,
                            container.asIObject());
                }
                                ...
                                try {
                                    ...
                                    if (container.getId() <= 0) { 
                                        ...
                                        createdData = object.hasObjectBeenCreated(
                                                container, ctx);
                                        if (createdData == null) {
                                            project = datamanager.saveAndReturnObject(
                                    ...
                                    link = (ProjectDatasetLink) 
                                            ...
                                            link = (ProjectDatasetLink) 
                                                    datamanager.saveAndReturnObject(ctx, link,
                                                    parameters, userName);
                                } catch (Exception e) {
                                    LogMessage msg = new LogMessage();
                                    msg.print("Cannot create the container " +
        ...
        throws ImportException
    {
        ...
        try {
            ...
            if(object.isOverrideName()) {
                String name = Utils.getDisplayedFileName(file.getAbsolutePath(), object.getDepthForName());
                ...
                try {
                    ...
                    if (icContainers.size() == 0) {
                        ...
                        if (label.isMarkedAsCancel())
                            ...
                            label.setCallback(importImageFile(ctx,
                ...
                if (container != null) {
                    if (container.getId() <= 0) { 
                        ...
                        createdData = object.hasObjectBeenCreated(
                                container, ctx);
                        if (createdData == null) {
                            project = datamanager.saveAndReturnObject(ctx,
                                    container.asIObject(), parameters,
                                    userName);
                            //register
                            object.addNewDataObject(
