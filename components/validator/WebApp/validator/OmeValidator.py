...
SCHEMA_DIR = cherrypy.config.get(
    "validator.schema", os.path.join(os.getcwd(), "schema"))
...
class XmlReport(object):
    ...
    def loadChoosenSchema(self):
                        ...
                        if self.theNamespace == schema_ns % "2009-09":
                            ...
                            if self.theNamespace == schema_ns % "2010-04":
                                ...
                                if self.theNamespace == schema_ns % "2010-06":
                                    ...
                                    if (self.theNamespace == schema_ns %
                                            "2011-06"):
                                        # use June 2010 schema
                                        self.theSchemaFile = \
                                            "ome-2011-06-V1.xsd"
        ...
        try:
            schema = etree.XMLSchema(etree.parse(
                schemaFilePath(self.theSchemaFile)))
        except:
            # chosen schema failed to laod
            self.errorList.append(ParseMessage(
                None, None, None, "XSD", None,
...
if __name__ == '__main__':
    for aFilename in [
            ...
            "samples/completesamplenoenc.xml", "samples/sdub.ome",
            "samples/sdub-fix.ome", "samples/sdub-fix-pre.ome",
            "samples/tiny.ome", "samples/broke.ome",
            "samples/tiny2008-02-V1.ome", "samples/tiny2008-09-V1.ome",
            "samples/tiny2009-09-V1.ome", "samples/fail2009-09-V1.ome"]:
