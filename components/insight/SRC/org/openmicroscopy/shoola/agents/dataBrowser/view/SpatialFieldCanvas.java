...
/**
 * Spatial display of all the fields for a given well.
 *
 ...
 * @since 3.0-Beta4
 */
class SpatialFieldCanvas extends WellFieldsCanvas {
     ...
     * @param x
     *            The location on the X-axis of a unit.
     * @param y
     *            The location on the Y-axis of a unit.
     */
    private void drawGrid(Graphics2D g2D, int x, int y) {
        ...
        if (x != 0)
            n = n / x;
        for (int i = 1; i <= n; i++) {
            g2D.drawLine(w / 2 + x * i, h / 2 - TICK, w / 2 + x * i, h / 2
                    + TICK);
        ...
        if (y != 0)
            n = n / y;
        for (int i = 1; i <= n; i++) {
            g2D.drawLine(w / 2 - TICK, h / 2 + y * i, w / 2 + TICK, h / 2 + y
                    * i);
            g2D.drawLine(w / 2 - TICK, h / 2 - y * i, w / 2 + TICK, h / 2 - y
                    * i);
        }
     ...
     */
    public SpatialFieldCanvas(WellFieldsView parent) {
        super(parent);
