...
--
  ...
  CREATE INDEX i_Fileset_clientVersionInfo ON fileset(clientVersionInfo);
  CREATE INDEX i_Fileset_serverVersionInfo ON fileset(serverVersionInfo);
  CREATE INDEX i_filesetactivity_owner ON filesetactivity(owner_id);
  CREATE INDEX i_filesetactivity_group ON filesetactivity(group_id);
  CREATE INDEX i_FilesetActivity_filesetActivityType ON filesetactivity(filesetActivityType);
  CREATE INDEX i_FilesetActivity_fileset ON filesetactivity(fileset);
  CREATE INDEX i_FilesetActivity_job ON filesetactivity(job);
...
CREATE SEQUENCE seq_filesetactivity; INSERT INTO _lock_ids (name, id) SELECT 'seq_filesetactivity', nextval('_lock_seq');
CREATE SEQUENCE seq_filesetactivitytype; INSERT INTO _lock_ids (name, id) SELECT 'seq_filesetactivitytype', nextval('_lock_seq');
    ...
    BEGIN
        ...
        IF eid = 0 OR eid IS NULL THEN
            ...
            INSERT INTO event (id, permissions, status, time, experimenter, experimentergroup, session, type)
                SELECT eid, -52, ''TRIGGERED'', clock_timestmap(), 0, 0, 0, 0;
        END IF;
        RETURN eid;
    END;'
LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION annotation_update_event_trigger() RETURNS "trigger"
    ...
    BEGIN
        ...
        FOR rec IN SELECT id, parent FROM filesetannotationlink WHERE child = new.id LOOP
            INSERT INTO _updated_annotations (entityid, entitytype) values (rec.parent, ''ome.model.core.Fileset'');
        END LOOP;
...
CREATE TRIGGER fileset_annotation_link_event_trigger
        ...
        FOR EACH ROW
        EXECUTE PROCEDURE annotation_link_event_trigger('ome.model.core.Fileset');
...
CREATE TRIGGER fileset_annotation_link_delete_trigger
        ...
        FOR EACH ROW
        EXECUTE PROCEDURE annotation_link_delete_trigger('ome.model.core.Fileset');
...
insert into dbpatch (currentVersion, currentPatch, previousVersion, previousPatch, message)
             values ('OMERO4.5DEV',  1,    'OMERO4.5DEV',   0,             'Initializing');
...
insert into filesetactivitytype (id,permissions,value)
    select ome_nextval('seq_filesetactivitytype'),-52,'Upload';
insert into filesetactivitytype (id,permissions,value)
    select ome_nextval('seq_filesetactivitytype'),-52,'Import';
insert into filesetactivitytype (id,permissions,value)
    select ome_nextval('seq_filesetactivitytype'),-52,'Thumbnails';
insert into filesetactivitytype (id,permissions,value)
    select ome_nextval('seq_filesetactivitytype'),-52,'Pyramids';
insert into filesetactivitytype (id,permissions,value)
    select ome_nextval('seq_filesetactivitytype'),-52,'Checksum';
insert into filesetactivitytype (id,permissions,value)
    select ome_nextval('seq_filesetactivitytype'),-52,'IntegrityCheck';
insert into filesetactivitytype (id,permissions,value)
    select ome_nextval('seq_filesetactivitytype'),-52,'Other';
insert into filesetactivitytype (id,permissions,value)
    select ome_nextval('seq_filesetactivitytype'),-52,'Unknown';
