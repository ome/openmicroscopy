    ...
    create table fileset (
        ...
        permissions int8 not null,
        version int4,
        clientVersionInfo int8 not null,
        creation_id int8 not null,
        external_id int8 unique,
        group_id int8 not null,
        owner_id int8 not null,
        update_id int8 not null,
        serverVersionInfo int8 not null,
        primary key (id)
    );;

    create table filesetactivity (
        id int8 not null,
        description text,
        permissions int8 not null,
        hideUntilDone bool not null,
        version int4,
        creation_id int8 not null,
        external_id int8 unique,
        group_id int8 not null,
        ...
        filesetActivityType int8 not null,
        job int8 not null,
        primary key (id)
    );;

    create table filesetactivitytype (
        id int8 not null,
        permissions int8 not null,
        value varchar(255) not null unique,
        external_id int8 unique,
    ...
    alter table fileset
        add constraint FKfileset_clientVersionInfo_filesetversioninfo
        foreign key (clientVersionInfo)
    ...
    alter table fileset
        add constraint FKfileset_serverVersionInfo_filesetversioninfo
        foreign key (serverVersionInfo)
        references filesetversioninfo  ;;
    ...
    alter table filesetactivity
        add constraint FKfilesetactivity_creation_id_event
        foreign key (creation_id)
        references event  ;;

    alter table filesetactivity
        add constraint FKfilesetactivity_update_id_event
        foreign key (update_id)
        references event  ;;

    alter table filesetactivity
        add constraint FKfilesetactivity_external_id_externalinfo
        foreign key (external_id)
        references externalinfo  ;;

    alter table filesetactivity
        add constraint FKfilesetactivity_fileset_fileset
        foreign key (fileset)
        references fileset  ;;

    alter table filesetactivity
        add constraint FKfilesetactivity_group_id_experimentergroup
        foreign key (group_id)
        references experimentergroup  ;;

    alter table filesetactivity
        add constraint FKfilesetactivity_owner_id_experimenter
        foreign key (owner_id)
        references experimenter  ;;

    alter table filesetactivity
        add constraint FKfilesetactivity_job_job
        foreign key (job)
        references job  ;;

    alter table filesetactivity
        add constraint FKfilesetactivity_filesetActivityType_filesetactivitytype
        foreign key (filesetActivityType)
        references filesetactivitytype  ;;

    alter table filesetactivitytype
        add constraint FKfilesetactivitytype_external_id_externalinfo
        foreign key (external_id)
        references externalinfo  ;;
