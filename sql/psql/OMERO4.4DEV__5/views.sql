...
-- with functional views.
  ...
  DROP TABLE count_LightPath_emissionFilterLinks_by_owner;

  CREATE OR REPLACE VIEW count_LightPath_emissionFilterLinks_by_owner (LightPath_id, owner_id, count) AS select parent, owner_id, count(*)
    FROM lightpathemissionfilterlink GROUP BY parent, owner_id ORDER BY parent;

  DROP TABLE count_LightPath_excitationFilterLinks_by_owner;

  CREATE OR REPLACE VIEW count_LightPath_excitationFilterLinks_by_owner (LightPath_id, owner_id, count) AS select parent, owner_id, count(*)
  ...
  DROP TABLE count_Image_microbeamManipulationLinks_by_owner;

  CREATE OR REPLACE VIEW count_Image_microbeamManipulationLinks_by_owner (Image_id, owner_id, count) AS select parent, owner_id, count(*)
    FROM imagemicrobeammanipulationlink GROUP BY parent, owner_id ORDER BY parent;

  DROP TABLE count_Image_roiLinks_by_owner;

  CREATE OR REPLACE VIEW count_Image_roiLinks_by_owner (Image_id, owner_id, count) AS select parent, owner_id, count(*)
    FROM imageroilink GROUP BY parent, owner_id ORDER BY parent;
  ...
  DROP TABLE count_Pixels_originalFileLinks_by_owner;

  CREATE OR REPLACE VIEW count_Pixels_originalFileLinks_by_owner (Pixels_id, owner_id, count) AS select child, owner_id, count(*)
    FROM originalfilepixelslink GROUP BY child, owner_id ORDER BY child;
  ...
  DROP TABLE count_ROI_imageLinks_by_owner;

  CREATE OR REPLACE VIEW count_ROI_imageLinks_by_owner (ROI_id, owner_id, count) AS select child, owner_id, count(*)
    FROM imageroilink GROUP BY child, owner_id ORDER BY child;

  DROP TABLE count_ROI_microbeamManipulationLinks_by_owner;

  CREATE OR REPLACE VIEW count_ROI_microbeamManipulationLinks_by_owner (ROI_id, owner_id, count) AS select child, owner_id, count(*)
    FROM microbeammanipulationroilink GROUP BY child, owner_id ORDER BY child;
  ...
  DROP TABLE count_Plane_annotationLinks_by_owner;

  CREATE OR REPLACE VIEW count_Plane_annotationLinks_by_owner (Plane_id, owner_id, count) AS select parent, owner_id, count(*)
    FROM planeannotationlink GROUP BY parent, owner_id ORDER BY parent;

  DROP TABLE count_OriginalFile_jobLinks_by_owner;

  CREATE OR REPLACE VIEW count_OriginalFile_jobLinks_by_owner (OriginalFile_id, owner_id, count) AS select child, owner_id, count(*)
    FROM joboriginalfilelink GROUP BY child, owner_id ORDER BY child;

  DROP TABLE count_OriginalFile_pixelsLinks_by_owner;

  CREATE OR REPLACE VIEW count_OriginalFile_pixelsLinks_by_owner (OriginalFile_id, owner_id, count) AS select parent, owner_id, count(*)
    FROM originalfilepixelslink GROUP BY parent, owner_id ORDER BY parent;
  ...
  DROP TABLE count_FilterSet_emissionFilterLinks_by_owner;

  CREATE OR REPLACE VIEW count_FilterSet_emissionFilterLinks_by_owner (FilterSet_id, owner_id, count) AS select parent, owner_id, count(*)
    FROM filtersetemissionfilterlink GROUP BY parent, owner_id ORDER BY parent;

  DROP TABLE count_FilterSet_excitationFilterLinks_by_owner;

  CREATE OR REPLACE VIEW count_FilterSet_excitationFilterLinks_by_owner (FilterSet_id, owner_id, count) AS select parent, owner_id, count(*)
    FROM filtersetexcitationfilterlink GROUP BY parent, owner_id ORDER BY parent;

  DROP TABLE count_MicrobeamManipulation_imageLinks_by_owner;

  CREATE OR REPLACE VIEW count_MicrobeamManipulation_imageLinks_by_owner (MicrobeamManipulation_id, owner_id, count) AS select child, owner_id, count(*)
    FROM imagemicrobeammanipulationlink GROUP BY child, owner_id ORDER BY child;

  DROP TABLE count_MicrobeamManipulation_roiLinks_by_owner;

  CREATE OR REPLACE VIEW count_MicrobeamManipulation_roiLinks_by_owner (MicrobeamManipulation_id, owner_id, count) AS select parent, owner_id, count(*)
    FROM microbeammanipulationroilink GROUP BY parent, owner_id ORDER BY parent;
  ...
  DROP TABLE count_Filter_filterSetExcitationFilterLinks_by_owner;

  CREATE OR REPLACE VIEW count_Filter_filterSetExcitationFilterLinks_by_owner (Filter_id, owner_id, count) AS select child, owner_id, count(*)
    FROM filtersetexcitationfilterlink GROUP BY child, owner_id ORDER BY child;

  DROP TABLE count_Filter_filterSetEmissionFilterLinks_by_owner;

  CREATE OR REPLACE VIEW count_Filter_filterSetEmissionFilterLinks_by_owner (Filter_id, owner_id, count) AS select child, owner_id, count(*)
    FROM filtersetemissionfilterlink GROUP BY child, owner_id ORDER BY child;

  DROP TABLE count_Filter_lightPathExcitationFilterLinks_by_owner;

  CREATE OR REPLACE VIEW count_Filter_lightPathExcitationFilterLinks_by_owner (Filter_id, owner_id, count) AS select child, owner_id, count(*)
    FROM lightpathexcitationfilterlink GROUP BY child, owner_id ORDER BY child;

  DROP TABLE count_Filter_lightPathEmissionFilterLinks_by_owner;

  CREATE OR REPLACE VIEW count_Filter_lightPathEmissionFilterLinks_by_owner (Filter_id, owner_id, count) AS select child, owner_id, count(*)
    FROM lightpathemissionfilterlink GROUP BY child, owner_id ORDER BY child;
