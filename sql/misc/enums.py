...
def connect(db1, user, password):
  conn1 = psycopg2.connect("host='localhost' dbname='%s' user='%s' password='%s'" % (db1,user,password))
  return conn1

def list(conn, table):
   cur = conn.cursor()
   cur.execute("""select * from pg_tables where schemaname='public' and tablename ilike '%s'""" % table)
