...
import org.openmicroscopy.shoola.util.ui.measurement.MeasurementComponent;
    ...
    extends TopWindow
{
	...
	/** Measurement Tool component which will control the creation of ROIs. */
	private MeasurementComponent measurement;
    ...
    /** 
     * First time the measurement tool  has been shown then variable 
     * <code>true</code>. 
     **/
    private boolean 		firstTimeMeasurementToolShown = true;
    ...
    private JMenu createControlsMenu()
    {
        ...
        item.setText(action.getName());
        menu.add(item);
        action = controller.getAction(ImViewerControl.MEASUREMENTTOOL);
        item = new JMenuItem(action);
        item.setText(action.getName());
        menu.add(item);
    ...
    /**
     * Sets the measurementTool visibility and displays the measurement tool on 
     * the main canvas.
     * 
     * @param b Pass <code>true</code> to display the measurement tool, 
     ...
     * @return See above
     */
    boolean hasMeasurementImage() { return (measurement != null); }
    ...
    /**
     * Returns <code>true</code> if the measurement tool is visible, 
     * <code>false</code> otherwise.
     * 
     * @return see above.
     */
    boolean isMeasurementToolVisible()
    {
        if (measurement != null) return measurement.isVisible();
        return false;
    }

    /**
     * Sets the measurement tool's visibility. If the measurement tool hasn't 
     * previously been created, we first create the measurement tool.
     * 
     * @param b Pass <code>true</code> to display the measurment, 
     * <code>false</code> otherwise.
     */
    void setMeasurementToolVisible(boolean b)
    {
        if (measurement == null) {
        	ImageModel imageModel = new ImageModel();
        	ArrayList<Color> channelColours = new ArrayList<Color>();
        	for(int i = 0 ; i < model.getChannelData().length; i++)
        	{
        		channelColours.add(model.getChannelColor(i));
        	}
        	imageModel.setChannelData(model.getChannelData(),channelColours);
        	imageModel.setImageDimensions(model.getMaxX(), model.getMaxY());
        	imageModel.setMaxT(model.getMaxT());
        	imageModel.setMaxZ(model.getMaxZ());
        	imageModel.setMicronsPixelX(model.getPixelsSizeX());
        ...
        if (!firstTimeMeasurementToolShown) {
        	...
        	//browser.removeComponent(measure.getMeasurementUI(), ImViewer.GRID_INDEX);
        }
        switch (model.getTabbedIndex()) {
			case ImViewer.VIEW_INDEX:
				showMeasurementTool(b);
				break;
			case ImViewer.GRID_INDEX:
				// Not covered!!!!
				// showMeasurementToolOnGrid(b);
				break;
		}
    }
