/*
 * org.openmicroscopy.shoola.util.math.geom2D.Point
 *
 *------------------------------------------------------------------------------
 *
...
public class Point
{
    ...
    {
        Point tp = vec(p);  //The vector associated to this point and p.
     ...
     */
    public Point sum(Point vec)
    {
        if (vec == null) throw new NullPointerException("No vector.");
        return new Point(x1+vec.x1, x2+vec.x2);
    }
     ...
     */
    public Point vec(Point p)
    {
        if (p == null) throw new NullPointerException("No point.");
        return new Point(p.x1-x1, p.x2-x2);
    }
