...
This script demonstrates a number of features that must be included in OMERO scripts,
as well as some that improve the usability etc.
 * Definition of script name, description and parameters
 * Inputs designed to be auto-populated based on currently selected objects in OMERO.insight client
 * Outputs designed to display a useful message and provide link to Image in Insight (Datasets handled too)
...
def editDescriptions(conn, scriptParams):
    ...
    """
    # we know scriptParams will have "Data_Type" and "IDs" since these parameters are not optional
    dataType = scriptParams["Data_Type"]
    ids = scriptParams["IDs"]

    # dataType is 'Dataset' or 'Image' so we can use it directly in getObjects()
    ...
    editedImgIds = []     # keep track of what we've edited, to provide feedback message
    for i in images:
...
See http://trac.openmicroscopy.org.uk/ome/wiki/OmeroPy/ScriptingServiceGuide for the tutorial that uses this script.""",
    ...
    scripts.String("New_Description", grouping="3",
        description="The new description to set for each Image in the Dataset"),
    )
