/*
 * org.openmicroscopy.shoola.agents.history.data.HistoryNode
 *
 *------------------------------------------------------------------------------
 *
 ...
 * @since OME2.2
 */
public class HistoryNode  implements GraphLayoutNode
{
	...
	private HistoryModuleData module;
	
	public HistoryNode(HistoryModuleData module) {
		this.module = module;
	}
	...
	/*private void buildSuccessors() {
		...
		while (iter.hasNext()) {
			...
			HistoryNode node = (HistoryNode) link.getToNode();
	...
	/*private void buildPredecessors() {
		...
		while (iter.hasNext()) {
			LayoutLinkData link = (LayoutLinkData) iter.next();
			HistoryNode node = (HistoryNode) link.getFromNode();
	...
	public HistoryModuleData getModule() {
		return module;
	}
}
